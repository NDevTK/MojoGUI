  ...
  const MojoParser = {
    ...
    parseBinding(content) {
      ...
      try {
        ...
        // Extract structs
        const structPattern =
          /(\w+(?:\.\w+)*)\s*=\s*class\s*\{\s*constructor\s*\(values\s*=\s*\{\}\)\s*\{([^}]+)\}/g;
        while ((match = structPattern.exec(content)) !== null) {
          ...
          if (fields.length > 0) {
            ...
            if (
              !structName.endsWith("Ptr") &&
              !structName.endsWith("Request")
            ) {
              result.structs.push({
                name: structName,
                fullName: match[1],
    ...
    extractMethods(content, classStart) {
      ...
      while ((match = methodPattern.exec(classBody)) !== null) {
        ...
        const methodBodyMatch = classBody
          .substring(methodStart)
          .match(/\{([^}]*)\}/);
        const hasReturn =
          methodBodyMatch && methodBodyMatch[1].includes("Promise");
    ...
    getInputType(type) {
      if (!type || typeof type !== "string") {
        if (type && type.type)
          type = type.type; // Extraction for MojoGUI Enum objects
        else return "text";
      }
      const typeLower = type.toLowerCase();
