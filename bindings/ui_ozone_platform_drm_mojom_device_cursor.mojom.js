  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ui.ozone.mojom.DeviceCursor_SetCursor_ParamsSpec.$.structSpec;
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ui.ozone.mojom.DeviceCursor_MoveCursor_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for MoveCursor failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
