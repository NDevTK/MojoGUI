  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_ToggleImeTray_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ToggleImeTray failed with TypeError');
           }
        }
        // Try Method 1: TogglePaletteTray
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_TogglePaletteTray_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for TogglePaletteTray failed with TypeError');
           }
        }
        // Try Method 2: ToggleLogoutTray
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_ToggleLogoutTray_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ToggleLogoutTray failed with TypeError');
           }
        }
        // Try Method 3: ToggleVirtualKeyboardTray
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_ToggleVirtualKeyboardTray_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ToggleVirtualKeyboardTray failed with TypeError');
           }
        }
        // Try Method 4: ToggleDictationTray
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_ToggleDictationTray_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ToggleDictationTray failed with TypeError');
           }
        }
        // Try Method 5: ToggleVideoConferenceTray
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_ToggleVideoConferenceTray_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ToggleVideoConferenceTray failed with TypeError');
           }
        }
        // Try Method 6: ToggleAnnotationTray
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_ToggleAnnotationTray_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ToggleAnnotationTray failed with TypeError');
           }
        }
        // Try Method 7: SetIsInUserChildSession
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_SetIsInUserChildSession_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetIsInUserChildSession failed with TypeError');
           }
        }
        // Try Method 8: TriggerPrivacyIndicators
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_TriggerPrivacyIndicators_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for TriggerPrivacyIndicators failed with TypeError');
           }
        }
        // Try Method 9: ResetHmrConsentStatus
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_ResetHmrConsentStatus_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ResetHmrConsentStatus failed with TypeError');
           }
        }
        // Try Method 10: SetBatteryIcon
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_SetBatteryIcon_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetBatteryIcon failed with TypeError');
           }
        }
        // Try Method 11: SetBatteryPercent
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.mojom.status_area_internals.PageHandler_SetBatteryPercent_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetBatteryPercent failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
