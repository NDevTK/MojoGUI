  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media_router.mojom.Debugger_ShouldFetchMirroringStats_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ShouldFetchMirroringStats failed with TypeError');
           }
        }
        // Try Method 1: OnMirroringStats
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media_router.mojom.Debugger_OnMirroringStats_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnMirroringStats failed with TypeError');
           }
        }
        // Try Method 2: BindReceiver
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media_router.mojom.Debugger_BindReceiver_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
