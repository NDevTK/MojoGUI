      ...
      switch (dispatchId) {
        case 0: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              ...
              const responder = mojo.internal.interfaceSupport.createResponder(this.endpoint, header.requestId, media.mojom.AudioEncoder_Initialize_ResponseParamsSpec, header, rawHeader);
               responder(response);
        ...
        case 1: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              const rawHeader = (args[2] && args[2].slice) ? args[2].slice(0, header.headerSize) : null;
              const responder = mojo.internal.interfaceSupport.createResponder(this.endpoint, header.requestId, media.mojom.AudioEncoder_Encode_ResponseParamsSpec, header, rawHeader);
               responder(response);
        ...
        case 2: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              const rawHeader = (args[2] && args[2].slice) ? args[2].slice(0, header.headerSize) : null;
              const responder = mojo.internal.interfaceSupport.createResponder(this.endpoint, header.requestId, media.mojom.AudioEncoder_Flush_ResponseParamsSpec, header, rawHeader);
               responder(response);
