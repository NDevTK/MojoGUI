  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.LocalStorageControl_BindStorageArea_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for BindStorageArea failed with TypeError');
           }
        }
        // Try Method 1: GetUsage
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.LocalStorageControl_GetUsage_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for GetUsage failed with TypeError');
           }
        }
        // Try Method 2: DeleteStorage
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.LocalStorageControl_DeleteStorage_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for DeleteStorage failed with TypeError');
           }
        }
        // Try Method 3: CleanUpStorage
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.LocalStorageControl_CleanUpStorage_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for CleanUpStorage failed with TypeError');
           }
        }
        // Try Method 4: Flush
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.LocalStorageControl_Flush_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.LocalStorageControl_PurgeMemory_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for PurgeMemory failed with TypeError');
           }
        }
        // Try Method 6: ApplyPolicyUpdates
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.LocalStorageControl_ApplyPolicyUpdates_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.LocalStorageControl_ForceKeepSessionState_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ForceKeepSessionState failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
