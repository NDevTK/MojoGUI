...
blink.mojom.AISummarizerReceiver = class {
  constructor(impl) {
    ...
    const ordinals = window.mojoScrambler.getOrdinals('AISummarizer', [
      { explicit: null },
      { explicit: null },
    ]);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      switch (dispatchId) {
        case 0: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(blink.mojom.AISummarizer_Summarize_ParamsSpec);
        ...
        case 1: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(blink.mojom.AISummarizer_MeasureUsage_ParamsSpec);
