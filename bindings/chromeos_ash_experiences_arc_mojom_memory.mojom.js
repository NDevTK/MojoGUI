...
mojo.internal.bindings.arc.mojom.MemoryInstanceSpec = { $ : {} };
...
mojo.internal.bindings.arc.mojom.MemoryInstanceRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('arc.mojom.MemoryInstance', [
      { explicit: 0 },
      { explicit: 3 },
    ]);
  }
...
mojo.internal.bindings.arc.mojom.MemoryInstanceReceiver = class {
  constructor(impl) {
    ...
    const ordinals = window.mojoScrambler.getOrdinals('arc.mojom.MemoryInstance', [
      { explicit: 0 },
      { explicit: 3 },
    ]);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.arc.mojom.MemoryInstance_DropCaches_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for DropCaches failed with TypeError');
           }
        }
        // Try Method 1: Reclaim
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.arc.mojom.MemoryInstance_Reclaim_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for Reclaim failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
