...
mojo.internal.bindings.blink.mojom.BytesProviderRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('blink.mojom.BytesProvider', [
      { explicit: null },
      { explicit: null },
      { explicit: null },
    ]);
...
mojo.internal.bindings.blink.mojom.BytesProviderReceiver = class {
  constructor(impl) {
    ...
    const ordinals = window.mojoScrambler.getOrdinals('blink.mojom.BytesProvider', [
      { explicit: null },
      { explicit: null },
      { explicit: null },
    ]);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.BytesProvider_RequestAsReply_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for RequestAsReply failed with TypeError');
           }
        }
        // Try Method 1: RequestAsStream
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.BytesProvider_RequestAsStream_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for RequestAsStream failed with TypeError');
           }
        }
        // Try Method 2: RequestAsFile
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.BytesProvider_RequestAsFile_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for RequestAsFile failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
