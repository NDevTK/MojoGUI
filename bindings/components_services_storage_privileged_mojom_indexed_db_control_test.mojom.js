  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.MockFailureInjector_FailOperation_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for FailOperation failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.IndexedDBControlTest_GetBaseDataPathForTesting_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for GetBaseDataPathForTesting failed with TypeError');
           }
        }
        // Try Method 1: GetFilePathForTesting
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.IndexedDBControlTest_GetFilePathForTesting_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for GetFilePathForTesting failed with TypeError');
           }
        }
        // Try Method 2: ResetCachesForTesting
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.IndexedDBControlTest_ResetCachesForTesting_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ResetCachesForTesting failed with TypeError');
           }
        }
        // Try Method 3: GetPathForBlobForTesting
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.IndexedDBControlTest_GetPathForBlobForTesting_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for GetPathForBlobForTesting failed with TypeError');
           }
        }
        // Try Method 4: FlushBackingStoreForTesting
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.IndexedDBControlTest_FlushBackingStoreForTesting_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for FlushBackingStoreForTesting failed with TypeError');
           }
        }
        // Try Method 5: FlushBucketSequenceForTesting
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.IndexedDBControlTest_FlushBucketSequenceForTesting_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for FlushBucketSequenceForTesting failed with TypeError');
           }
        }
        // Try Method 6: GetUsageForTesting
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.IndexedDBControlTest_GetUsageForTesting_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for GetUsageForTesting failed with TypeError');
           }
        }
        // Try Method 7: BindMockFailureSingletonForTesting
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.IndexedDBControlTest_BindMockFailureSingletonForTesting_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for BindMockFailureSingletonForTesting failed with TypeError');
           }
        }
        // Try Method 8: ForceInitializeFromFilesForTesting
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.storage.mojom.IndexedDBControlTest_ForceInitializeFromFilesForTesting_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ForceInitializeFromFilesForTesting failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
             console.warn('[GeneratedReceiver] Failed to discover ordinal ' + header.ordinal);
