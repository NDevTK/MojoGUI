  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.patch.mojom.FilePatcher_PatchFilePuffPatch_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for PatchFilePuffPatch failed with TypeError');
           }
        }
        // Try Method 1: PatchFileZucchini
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.patch.mojom.FilePatcher_PatchFileZucchini_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for PatchFileZucchini failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
