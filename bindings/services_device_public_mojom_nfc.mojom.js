  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.NFC_SetClient_ParamsSpec.$.structSpec;
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.NFC_Push_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for Push failed with TypeError');
           }
        }
        // Try Method 2: CancelPush
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.NFC_CancelPush_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for CancelPush failed with TypeError');
           }
        }
        // Try Method 3: MakeReadOnly
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.NFC_MakeReadOnly_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for MakeReadOnly failed with TypeError');
           }
        }
        // Try Method 4: CancelMakeReadOnly
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.NFC_CancelMakeReadOnly_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for CancelMakeReadOnly failed with TypeError');
           }
        }
        // Try Method 5: Watch
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.NFC_Watch_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for Watch failed with TypeError');
           }
        }
        // Try Method 6: CancelWatch
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.NFC_CancelWatch_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for CancelWatch failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.NFCClient_OnWatch_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnWatch failed with TypeError');
           }
        }
        // Try Method 1: OnError
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.NFCClient_OnError_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.RawNFCClient_OnWatch_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.RawNFCClient_OnError_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
