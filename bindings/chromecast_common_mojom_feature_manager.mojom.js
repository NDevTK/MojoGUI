...
mojo.internal.bindings.chromecast.shell.mojom.FeatureManagerRemote = class {
  ...
  constructor(handle = undefined) {
    this.proxy = new mojo.internal.interfaceSupport.InterfaceRemoteBase(
      mojo.internal.bindings.chromecast.shell.mojom.FeatureManagerPendingReceiver,
      handle);
    this.$ = new mojo.internal.bindings.chromecast.shell.mojom.FeatureManagerRemoteCallHandler(this.proxy);
  }
  ...
  configureFeatures(arg_features) {
    return this.$.configureFeatures(arg_features);
  }
};

mojo.internal.bindings.chromecast.shell.mojom.FeatureManagerRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('chromecast.shell.mojom.FeatureManager', [
      { explicit: null },
    ]);
  }
...
mojo.internal.bindings.chromecast.shell.mojom.FeatureManager.getRemote = function() {
  let remote = new mojo.internal.bindings.chromecast.shell.mojom.FeatureManagerRemote();
...
mojo.internal.bindings.chromecast.shell.mojom.FeatureManagerReceiver = class {
  constructor(impl) {
    this.impl = impl;
    this.endpoint = null;
    this.ordinalMap = new Map();
    const ordinals = window.mojoScrambler.getOrdinals('chromecast.shell.mojom.FeatureManager', [
      { explicit: null },
    ]);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        // Try Method 0: ConfigureFeatures
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chromecast.shell.mojom.FeatureManager_ConfigureFeatures_ParamsSpec.$.structSpec;
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 ...
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> ConfigureFeatures (0)');
                 this.mapOrdinal(header.ordinal, 0);
                 dispatchId = 0;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for ConfigureFeatures failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
...
mojo.internal.bindings.chromecast.shell.mojom.FeatureManagerReceiver = mojo.internal.bindings.chromecast.shell.mojom.FeatureManagerReceiver;
