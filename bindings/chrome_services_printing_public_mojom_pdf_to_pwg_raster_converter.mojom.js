  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.printing.mojom.PdfToPwgRasterConverter_Convert_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for Convert failed with TypeError');
           }
        }
        // Try Method 1: SetUseSkiaRendererPolicy
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.printing.mojom.PdfToPwgRasterConverter_SetUseSkiaRendererPolicy_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetUseSkiaRendererPolicy failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
