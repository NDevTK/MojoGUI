  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             decoder.decodeStructInline(network.mojom.WebBundleHandle_Clone_ParamsSpec);
        ...
        if (dispatchId === undefined) {
           try {
             decoder.decodeStructInline(network.mojom.WebBundleHandle_OnWebBundleError_ParamsSpec);
        ...
        if (dispatchId === undefined) {
           try {
             decoder.decodeStructInline(network.mojom.WebBundleHandle_OnWebBundleLoadFinished_ParamsSpec);
      ...
      switch (dispatchId) {
        case 0: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(network.mojom.WebBundleHandle_Clone_ParamsSpec.$.structSpec);
          const result = this.impl.clone(params.receiver);
          break;
        }
        case 1: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(network.mojom.WebBundleHandle_OnWebBundleError_ParamsSpec.$.structSpec);
          const result = this.impl.onWebBundleError(params.type, params.message);
          break;
        }
        case 2: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(network.mojom.WebBundleHandle_OnWebBundleLoadFinished_ParamsSpec.$.structSpec);
