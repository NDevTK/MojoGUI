  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.network.mojom.ReportingApiObserver_OnReportAdded_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnReportAdded failed with TypeError');
           }
        }
        // Try Method 1: OnReportUpdated
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.network.mojom.ReportingApiObserver_OnReportUpdated_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnReportUpdated failed with TypeError');
           }
        }
        // Try Method 2: OnEndpointsUpdatedForOrigin
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.network.mojom.ReportingApiObserver_OnEndpointsUpdatedForOrigin_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnEndpointsUpdatedForOrigin failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
