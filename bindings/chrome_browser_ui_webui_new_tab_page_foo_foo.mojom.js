...
mojo.internal.bindings.foo.mojom.FooHandlerRemote = class {
  ...
  constructor(handle = undefined) {
    this.proxy = new mojo.internal.interfaceSupport.InterfaceRemoteBase(
      mojo.internal.bindings.foo.mojom.FooHandlerPendingReceiver,
      handle);
    this.$ = new mojo.internal.bindings.foo.mojom.FooHandlerRemoteCallHandler(this.proxy);
  }
...
mojo.internal.bindings.foo.mojom.FooHandlerRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('foo.mojom.FooHandler', [
      { explicit: null },
    ]);
  }
...
mojo.internal.bindings.foo.mojom.FooHandler.getRemote = function() {
  let remote = new mojo.internal.bindings.foo.mojom.FooHandlerRemote();
...
mojo.internal.bindings.foo.mojom.FooHandlerReceiver = class {
  constructor(impl) {
    this.impl = impl;
    this.endpoint = null;
    this.ordinalMap = new Map();
    const ordinals = window.mojoScrambler.getOrdinals('foo.mojom.FooHandler', [
      { explicit: null },
    ]);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        // Try Method 0: GetData
        if (dispatchId === undefined) {
           try {
             decoder.decodeStructInline(mojo.internal.bindings.foo.mojom.FooHandler_GetData_ParamsSpec.$.structSpec);
             console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> GetData (0)');
      ...
      switch (dispatchId) {
        case 0: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(mojo.internal.bindings.foo.mojom.FooHandler_GetData_ParamsSpec.$.structSpec);
          console.log('[GeneratedReceiver] Calling impl.getData');
          const result = this.impl.getData();
          const expectsResponse = header.expectsResponse || (header.flags & 1);
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              const rawHeader = (args[2] && args[2].slice) ? args[2].slice(0, header.headerSize) : null;
              const responder = mojo.internal.interfaceSupport.createResponder(this.endpoint, header.requestId, mojo.internal.bindings.foo.mojom.FooHandler_GetData_ResponseParamsSpec, header, rawHeader);
               responder(response);
...
mojo.internal.bindings.foo.mojom.FooHandlerReceiver = mojo.internal.bindings.foo.mojom.FooHandlerReceiver;
