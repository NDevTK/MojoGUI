  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.viz.mojom.LayeredWindowUpdater_OnAllocatedSharedMemory_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnAllocatedSharedMemory failed with TypeError');
           }
        }
        // Try Method 1: Draw
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.viz.mojom.LayeredWindowUpdater_Draw_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for Draw failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
