  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.content.mojom.FontCacheWin_PreCacheFont_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for PreCacheFont failed with TypeError');
           }
        }
        // Try Method 1: ReleaseCachedFonts
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.content.mojom.FontCacheWin_ReleaseCachedFonts_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ReleaseCachedFonts failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
