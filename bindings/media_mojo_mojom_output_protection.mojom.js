  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.OutputProtection_QueryStatus_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for QueryStatus failed with TypeError');
           }
        }
        // Try Method 1: EnableProtection
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.OutputProtection_EnableProtection_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for EnableProtection failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
      ...
      switch (dispatchId) {
        case 0: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              this.endpoint.send(header.ordinal, header.requestId, mojo.internal.kMessageFlagIsResponse, mojo.internal.bindings.media.mojom.OutputProtection_QueryStatus_ResponseParamsSpec, response);
        ...
        case 1: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              this.endpoint.send(header.ordinal, header.requestId, mojo.internal.kMessageFlagIsResponse, mojo.internal.bindings.media.mojom.OutputProtection_EnableProtection_ResponseParamsSpec, response);
