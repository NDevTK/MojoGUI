...
mojo.internal.bindings.heap_profiling.mojom.SnapshotControllerRemote = class {
  ...
  constructor(handle = undefined) {
    this.proxy = new mojo.internal.interfaceSupport.InterfaceRemoteBase(
      mojo.internal.bindings.heap_profiling.mojom.SnapshotControllerPendingReceiver,
      handle);
    this.$ = new mojo.internal.bindings.heap_profiling.mojom.SnapshotControllerRemoteCallHandler(this.proxy);
  }
  ...
  takeSnapshot(arg_process_probability_pct, arg_process_index) {
    return this.$.takeSnapshot(arg_process_probability_pct, arg_process_index);
  }
...
mojo.internal.bindings.heap_profiling.mojom.SnapshotControllerRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('heap_profiling.mojom.SnapshotController', [
      { explicit: null },
      { explicit: null },
    ]);
  }
...
mojo.internal.bindings.heap_profiling.mojom.SnapshotController.getRemote = function() {
  let remote = new mojo.internal.bindings.heap_profiling.mojom.SnapshotControllerRemote();
...
mojo.internal.bindings.heap_profiling.mojom.SnapshotControllerReceiver = class {
  constructor(impl) {
    this.impl = impl;
    this.endpoint = null;
    this.ordinalMap = new Map();
    const ordinals = window.mojoScrambler.getOrdinals('heap_profiling.mojom.SnapshotController', [
      { explicit: null },
      { explicit: null },
    ]);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.heap_profiling.mojom.SnapshotController_TakeSnapshot_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for TakeSnapshot failed with TypeError');
           }
        }
        // Try Method 1: LogMetricsWithoutSnapshot
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.heap_profiling.mojom.SnapshotController_LogMetricsWithoutSnapshot_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for LogMetricsWithoutSnapshot failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
      ...
      switch (dispatchId) {
        ...
        case 1: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(mojo.internal.bindings.heap_profiling.mojom.SnapshotController_LogMetricsWithoutSnapshot_ParamsSpec.$.structSpec);
...
mojo.internal.bindings.heap_profiling.mojom.SnapshotControllerReceiver = mojo.internal.bindings.heap_profiling.mojom.SnapshotControllerReceiver;
