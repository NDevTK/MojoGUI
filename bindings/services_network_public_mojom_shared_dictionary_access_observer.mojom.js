...
mojo.internal.Struct(
    mojo.internal.bindings.network.mojom.SharedDictionaryAccessDetailsSpec, 'network.mojom.SharedDictionaryAccessDetails', [
      ...
      mojo.internal.StructField('arg_is_blocked', 24, 0, mojo.internal.Bool, false, false, 0, undefined),
    ],
    [[0, 40]]);
...
mojo.internal.bindings.network.mojom.SharedDictionaryAccessObserverRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('network.mojom.SharedDictionaryAccessObserver', [
      { explicit: null },
      { explicit: null },
    ]);
  }
...
mojo.internal.bindings.network.mojom.SharedDictionaryAccessObserverReceiver = class {
  constructor(impl) {
    ...
    const ordinals = window.mojoScrambler.getOrdinals('network.mojom.SharedDictionaryAccessObserver', [
      { explicit: null },
      { explicit: null },
    ]);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.network.mojom.SharedDictionaryAccessObserver_OnSharedDictionaryAccessed_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnSharedDictionaryAccessed failed with TypeError');
           }
        }
        // Try Method 1: Clone
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.network.mojom.SharedDictionaryAccessObserver_Clone_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
