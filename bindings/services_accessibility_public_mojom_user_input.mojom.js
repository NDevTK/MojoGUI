  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             decoder.decodeStructInline(ax.mojom.UserInput_SendSyntheticKeyEventForShortcutOrNavigation_ParamsSpec);
        ...
        if (dispatchId === undefined) {
           try {
             decoder.decodeStructInline(ax.mojom.UserInput_SendSyntheticMouseEvent_ParamsSpec);
      ...
      switch (dispatchId) {
        case 0: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(ax.mojom.UserInput_SendSyntheticKeyEventForShortcutOrNavigation_ParamsSpec.$.structSpec);
          const result = this.impl.sendSyntheticKeyEventForShortcutOrNavigation(params.key_event);
          break;
        }
        case 1: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(ax.mojom.UserInput_SendSyntheticMouseEvent_ParamsSpec.$.structSpec);
