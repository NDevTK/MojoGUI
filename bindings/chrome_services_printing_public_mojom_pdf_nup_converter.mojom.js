...
mojo.internal.Struct(
    mojo.internal.bindings.printing.mojom.PdfNupConverter_NupPageConvert_ResponseParamsSpec, 'printing.mojom.PdfNupConverter_NupPageConvert_ResponseParams', [
      mojo.internal.StructField('arg_pdf_region', 0, 0, mojo.internal.bindings.mojo_base.mojom.ReadOnlySharedMemoryRegionSpec, null, true, 0, undefined),
...
mojo.internal.bindings.printing.mojom.PdfNupConverterRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('printing.mojom.PdfNupConverter', [
      { explicit: null },
      { explicit: null },
      { explicit: null },
      { explicit: null },
...
mojo.internal.bindings.printing.mojom.PdfNupConverterReceiver = class {
  constructor(impl) {
    ...
    const ordinals = window.mojoScrambler.getOrdinals('printing.mojom.PdfNupConverter', [
      { explicit: null },
      { explicit: null },
      { explicit: null },
      { explicit: null },
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.printing.mojom.PdfNupConverter_NupPageConvert_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for NupPageConvert failed with TypeError');
           }
        }
        // Try Method 1: NupDocumentConvert
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.printing.mojom.PdfNupConverter_NupDocumentConvert_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for NupDocumentConvert failed with TypeError');
           }
        }
        // Try Method 2: SetWebContentsURL
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.printing.mojom.PdfNupConverter_SetWebContentsURL_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.printing.mojom.PdfNupConverter_SetUseSkiaRendererPolicy_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
