...
mojo.internal.bindings.media.mojom.AudioLogRemote = class {
  ...
  constructor(handle = undefined) {
    this.proxy = new mojo.internal.interfaceSupport.InterfaceRemoteBase(
      ...
      handle);
    this.$ = new mojo.internal.bindings.media.mojom.AudioLogRemoteCallHandler(this.proxy);
  }
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.AudioLog_OnCreated_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnCreated failed with TypeError');
           }
        }
        // Try Method 1: OnStarted
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.AudioLog_OnStarted_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
        ...
        // Try Method 2: OnStopped
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.AudioLog_OnStopped_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> OnStopped (2)');
        ...
        // Try Method 3: OnClosed
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.AudioLog_OnClosed_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> OnClosed (3)');
                 this.mapOrdinal(header.ordinal, 3);
                 dispatchId = 3;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnClosed failed with TypeError');
           }
        }
        // Try Method 4: OnError
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.AudioLog_OnError_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
        ...
        // Try Method 5: OnSetVolume
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.AudioLog_OnSetVolume_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> OnSetVolume (5)');
                 this.mapOrdinal(header.ordinal, 5);
                 dispatchId = 5;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnSetVolume failed with TypeError');
           }
        }
        // Try Method 6: OnProcessingStateChanged
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.AudioLog_OnProcessingStateChanged_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> OnProcessingStateChanged (6)');
                 this.mapOrdinal(header.ordinal, 6);
                 dispatchId = 6;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnProcessingStateChanged failed with TypeError');
           }
        }
        // Try Method 7: OnLogMessage
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.AudioLog_OnLogMessage_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> OnLogMessage (7)');
                 this.mapOrdinal(header.ordinal, 7);
                 dispatchId = 7;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnLogMessage failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
      ...
      switch (dispatchId) {
        ...
        case 1: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(mojo.internal.bindings.media.mojom.AudioLog_OnStarted_ParamsSpec.$.structSpec);
        ...
        case 2: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(mojo.internal.bindings.media.mojom.AudioLog_OnStopped_ParamsSpec.$.structSpec);
        ...
        case 3: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(mojo.internal.bindings.media.mojom.AudioLog_OnClosed_ParamsSpec.$.structSpec);
        ...
        case 4: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(mojo.internal.bindings.media.mojom.AudioLog_OnError_ParamsSpec.$.structSpec);
...
mojo.internal.bindings.media.mojom.AudioLogReceiver = mojo.internal.bindings.media.mojom.AudioLogReceiver;

mojo.internal.bindings.media.mojom.AudioLogPtr = mojo.internal.bindings.media.mojom.AudioLogRemote;
mojo.internal.bindings.media.mojom.AudioLogRequest = mojo.internal.bindings.media.mojom.AudioLogPendingReceiver;
...
mojo.internal.bindings.media.mojom.AudioLogFactoryRemote = class {
  ...
  constructor(handle = undefined) {
    this.proxy = new mojo.internal.interfaceSupport.InterfaceRemoteBase(
      ...
      handle);
    this.$ = new mojo.internal.bindings.media.mojom.AudioLogFactoryRemoteCallHandler(this.proxy);
  }
  ...
  createAudioLog(arg_component, arg_component_id, arg_audio_log_receiver) {
    return this.$.createAudioLog(arg_component, arg_component_id, arg_audio_log_receiver);
  }
};
...
mojo.internal.bindings.media.mojom.AudioLogFactoryRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('media.mojom.AudioLogFactory', [
      { explicit: null },
    ]);
  }
...
mojo.internal.bindings.media.mojom.AudioLogFactoryReceiver = class {
  constructor(impl) {
    ...
    this.endpoint = null;
    this.ordinalMap = new Map();
    const ordinals = window.mojoScrambler.getOrdinals('media.mojom.AudioLogFactory', [
      { explicit: null },
    ]);
    ordinals.forEach((ord, idx) => {
      this.ordinalMap.set(ord, idx); // Scrambled/Explicit
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        // Try Method 0: CreateAudioLog
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.media.mojom.AudioLogFactory_CreateAudioLog_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> CreateAudioLog (0)');
                 this.mapOrdinal(header.ordinal, 0);
                 dispatchId = 0;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for CreateAudioLog failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
             console.warn('[GeneratedReceiver] Failed to discover ordinal ' + header.ordinal);
...
mojo.internal.bindings.media.mojom.AudioLogFactoryReceiver = mojo.internal.bindings.media.mojom.AudioLogFactoryReceiver;
