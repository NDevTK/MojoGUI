...
mojo.internal.Struct(
    mojo.internal.bindings.chrome.mojom.ImportedBookmarkEntrySpec, 'chrome.mojom.ImportedBookmarkEntry', [
      ...
      mojo.internal.StructField('arg_blocked_by_user', 4, 0, mojo.internal.Bool, false, false, 0, undefined),
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnImportStart_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnImportStart failed with TypeError');
           }
        }
        // Try Method 1: OnImportFinished
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnImportFinished_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnImportFinished failed with TypeError');
           }
        }
        // Try Method 2: OnImportItemStart
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnImportItemStart_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnImportItemStart failed with TypeError');
           }
        }
        // Try Method 3: OnImportItemFinished
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnImportItemFinished_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnImportItemFinished failed with TypeError');
           }
        }
        // Try Method 4: OnHistoryImportStart
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnHistoryImportStart_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnHistoryImportStart failed with TypeError');
           }
        }
        // Try Method 5: OnHistoryImportGroup
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnHistoryImportGroup_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnHistoryImportGroup failed with TypeError');
           }
        }
        // Try Method 6: OnHomePageImportReady
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnHomePageImportReady_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnHomePageImportReady failed with TypeError');
           }
        }
        // Try Method 7: OnBookmarksImportStart
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnBookmarksImportStart_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnBookmarksImportStart failed with TypeError');
           }
        }
        // Try Method 8: OnBookmarksImportGroup
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnBookmarksImportGroup_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnBookmarksImportGroup failed with TypeError');
           }
        }
        // Try Method 9: OnFaviconsImportStart
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnFaviconsImportStart_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnFaviconsImportStart failed with TypeError');
           }
        }
        // Try Method 10: OnFaviconsImportGroup
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnFaviconsImportGroup_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnFaviconsImportGroup failed with TypeError');
           }
        }
        // Try Method 11: OnPasswordFormImportReady
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnPasswordFormImportReady_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnPasswordFormImportReady failed with TypeError');
           }
        }
        // Try Method 12: OnKeywordsImportReady
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnKeywordsImportReady_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnKeywordsImportReady failed with TypeError');
           }
        }
        // Try Method 13: OnAutofillFormDataImportStart
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnAutofillFormDataImportStart_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnAutofillFormDataImportStart failed with TypeError');
           }
        }
        // Try Method 14: OnAutofillFormDataImportGroup
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImportObserver_OnAutofillFormDataImportGroup_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnAutofillFormDataImportGroup failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
...
mojo.internal.Struct(
    mojo.internal.bindings.chrome.mojom.ProfileImport_StartImport_ParamsSpec, 'chrome.mojom.ProfileImport_StartImport_Params', [
      ...
      mojo.internal.StructField('arg_items', 8, 0, mojo.internal.Uint16, 0, false, 0, undefined),
      mojo.internal.StructField('arg_localized_strings', 16, 0, mojo.internal.Map(mojo.internal.Uint32, mojo.internal.String, false), null, false, 0, undefined),
      mojo.internal.StructField('arg_observer', 24, 0, mojo.internal.InterfaceProxy(mojo.internal.bindings.chrome.mojom.ProfileImportObserverRemote), null, false, 0, undefined),
    ],
    [[0, 40]]);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImport_StartImport_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for StartImport failed with TypeError');
           }
        }
        // Try Method 1: CancelImport
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImport_CancelImport_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for CancelImport failed with TypeError');
           }
        }
        // Try Method 2: ReportImportItemFinished
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.chrome.mojom.ProfileImport_ReportImportItemFinished_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ReportImportItemFinished failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
             console.warn('[GeneratedReceiver] Failed to discover ordinal ' + header.ordinal);
