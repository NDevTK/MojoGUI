  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.printing.mojom.PrintingService_BindPdfNupConverter_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for BindPdfNupConverter failed with TypeError');
           }
        }
        // Try Method 1: BindPdfToPwgRasterConverter
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.printing.mojom.PrintingService_BindPdfToPwgRasterConverter_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
        ...
        if (dispatchId === undefined) {
           try {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for BindPdfToEmfConverterFactory failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
