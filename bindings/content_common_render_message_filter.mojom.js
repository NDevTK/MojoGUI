  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.content.mojom.RenderMessageFilter_GenerateSingleFrameRoutingInfo_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for GenerateSingleFrameRoutingInfo failed with TypeError');
           }
        }
        // Try Method 1: GenerateFrameRoutingInfos
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.content.mojom.RenderMessageFilter_GenerateFrameRoutingInfos_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for GenerateFrameRoutingInfos failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
