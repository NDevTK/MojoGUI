  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             decoder.decodeStruct(network.mojom.NetworkInterfaceChangeListener_OnNetworkInterfacesChanged_ParamsSpec.$, message.header.headerSize);
      ...
      switch (dispatchId) {
        case 0: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStruct(network.mojom.NetworkInterfaceChangeListener_OnNetworkInterfacesChanged_ParamsSpec.$, message.header.headerSize);
