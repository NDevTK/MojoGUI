...
mojo.internal.bindings.network.mojom.TrustTokenAccessObserverSpec = { $ : {} };
...
mojo.internal.bindings.network.mojom.TrustTokenAccessObserverRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('network.mojom.TrustTokenAccessObserver', [
      { explicit: null },
      { explicit: null },
    ]);
  }
...
mojo.internal.bindings.network.mojom.TrustTokenAccessObserverReceiver = class {
  constructor(impl) {
    ...
    const ordinals = window.mojoScrambler.getOrdinals('network.mojom.TrustTokenAccessObserver', [
      { explicit: null },
      { explicit: null },
    ]);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.network.mojom.TrustTokenAccessObserver_OnTrustTokensAccessed_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnTrustTokensAccessed failed with TypeError');
           }
        }
        // Try Method 1: Clone
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.network.mojom.TrustTokenAccessObserver_Clone_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
