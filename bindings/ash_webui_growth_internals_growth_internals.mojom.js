  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.growth.mojom.PageHandler_GetCampaignsLogs_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for GetCampaignsLogs failed with TypeError');
           }
        }
        // Try Method 1: ClearAllEvents
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.ash.growth.mojom.PageHandler_ClearAllEvents_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ClearAllEvents failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
