...
mojo.internal.bindings.blink.mojom.AnchorElementInteractionHostSpec = { $ : {} };
...
mojo.internal.bindings.blink.mojom.AnchorElementInteractionHostRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('blink.mojom.AnchorElementInteractionHost', [
      { explicit: null },
      { explicit: null },
      { explicit: null },
      { explicit: null },
...
mojo.internal.bindings.blink.mojom.AnchorElementInteractionHostReceiver = class {
  constructor(impl) {
    ...
    const ordinals = window.mojoScrambler.getOrdinals('blink.mojom.AnchorElementInteractionHost', [
      { explicit: null },
      { explicit: null },
      { explicit: null },
      { explicit: null },
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.AnchorElementInteractionHost_OnPointerDown_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnPointerDown failed with TypeError');
           }
        }
        // Try Method 1: OnPointerHoverEager
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.AnchorElementInteractionHost_OnPointerHoverEager_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnPointerHoverEager failed with TypeError');
           }
        }
        // Try Method 2: OnPointerHoverModerate
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.AnchorElementInteractionHost_OnPointerHoverModerate_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnPointerHoverModerate failed with TypeError');
           }
        }
        // Try Method 3: OnModerateViewportHeuristicTriggered
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.AnchorElementInteractionHost_OnModerateViewportHeuristicTriggered_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnModerateViewportHeuristicTriggered failed with TypeError');
           }
        }
        // Try Method 4: OnEagerViewportHeuristicTriggered
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.AnchorElementInteractionHost_OnEagerViewportHeuristicTriggered_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnEagerViewportHeuristicTriggered failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
