  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.tracing.mojom.TracingService_Initialize_ParamsSpec.$.structSpec;
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.tracing.mojom.TracingService_AddClient_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
