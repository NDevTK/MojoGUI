...
mojo.internal.bindings.discards.mojom.SiteDataProviderRemote = class {
  ...
  constructor(handle = undefined) {
    this.proxy = new mojo.internal.interfaceSupport.InterfaceRemoteBase(
      mojo.internal.bindings.discards.mojom.SiteDataProviderPendingReceiver,
      handle);
    this.$ = new mojo.internal.bindings.discards.mojom.SiteDataProviderRemoteCallHandler(this.proxy);
  }
  ...
  getSiteDataArray(arg_explicitly_requested_origins) {
    return this.$.getSiteDataArray(arg_explicitly_requested_origins);
  }
...
mojo.internal.bindings.discards.mojom.SiteDataProviderRemoteCallHandler = class {
  constructor(proxy) {
    this.proxy = proxy;
    this.ordinals = window.mojoScrambler.getOrdinals('discards.mojom.SiteDataProvider', [
      { explicit: null },
      { explicit: null },
    ]);
  }
...
mojo.internal.bindings.discards.mojom.SiteDataProvider.getRemote = function() {
  let remote = new mojo.internal.bindings.discards.mojom.SiteDataProviderRemote();
...
mojo.internal.bindings.discards.mojom.SiteDataProviderReceiver = class {
  constructor(impl) {
    this.impl = impl;
    this.endpoint = null;
    this.ordinalMap = new Map();
    const ordinals = window.mojoScrambler.getOrdinals('discards.mojom.SiteDataProvider', [
      { explicit: null },
      { explicit: null },
    ]);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        // Try Method 0: GetSiteDataArray
        if (dispatchId === undefined) {
           try {
             decoder.decodeStructInline(mojo.internal.bindings.discards.mojom.SiteDataProvider_GetSiteDataArray_ParamsSpec.$.structSpec);
             console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> GetSiteDataArray (0)');
        ...
        // Try Method 1: GetSiteDataDatabaseSize
        if (dispatchId === undefined) {
           try {
             decoder.decodeStructInline(mojo.internal.bindings.discards.mojom.SiteDataProvider_GetSiteDataDatabaseSize_ParamsSpec.$.structSpec);
             console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> GetSiteDataDatabaseSize (1)');
      ...
      switch (dispatchId) {
        case 0: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              ...
              const responder = mojo.internal.interfaceSupport.createResponder(this.endpoint, header.requestId, mojo.internal.bindings.discards.mojom.SiteDataProvider_GetSiteDataArray_ResponseParamsSpec, header, rawHeader);
               responder(response);
        ...
        case 1: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(mojo.internal.bindings.discards.mojom.SiteDataProvider_GetSiteDataDatabaseSize_ParamsSpec.$.structSpec);
          console.log('[GeneratedReceiver] Calling impl.getSiteDataDatabaseSize');
          const result = this.impl.getSiteDataDatabaseSize();
          const expectsResponse = header.expectsResponse || (header.flags & 1);
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              const rawHeader = (args[2] && args[2].slice) ? args[2].slice(0, header.headerSize) : null;
              const responder = mojo.internal.interfaceSupport.createResponder(this.endpoint, header.requestId, mojo.internal.bindings.discards.mojom.SiteDataProvider_GetSiteDataDatabaseSize_ResponseParamsSpec, header, rawHeader);
               responder(response);
...
mojo.internal.bindings.discards.mojom.SiteDataProviderReceiver = mojo.internal.bindings.discards.mojom.SiteDataProviderReceiver;
