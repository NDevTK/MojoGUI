  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.content.mojom.RendererHost_GetBrowserHistogram_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for GetBrowserHistogram failed with TypeError');
           }
        }
        // Try Method 1: SuddenTerminationAllowedChanged
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.content.mojom.RendererHost_SuddenTerminationAllowedChanged_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for SuddenTerminationAllowedChanged failed with TypeError');
           }
        }
        // Try Method 2: RecordUserMetricsAction
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.content.mojom.RendererHost_RecordUserMetricsAction_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
        ...
        if (dispatchId === undefined) {
           try {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for HasGpuProcess failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
