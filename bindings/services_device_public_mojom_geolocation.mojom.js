  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.Geolocation_SetHighAccuracyHint_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetHighAccuracyHint failed with TypeError');
           }
        }
        // Try Method 1: QueryNextPosition
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.device.mojom.Geolocation_QueryNextPosition_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for QueryNextPosition failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
