  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             decoder.decodeStructInline(blink.mojom.BadgeService_SetBadge_ParamsSpec);
        ...
        if (dispatchId === undefined) {
           try {
             decoder.decodeStructInline(blink.mojom.BadgeService_ClearBadge_ParamsSpec);
      ...
      switch (dispatchId) {
        case 0: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(blink.mojom.BadgeService_SetBadge_ParamsSpec.$.structSpec);
          const result = this.impl.setBadge(params.value);
          break;
        }
        case 1: {
          const decoder = new mojo.internal.Decoder(message.payload, message.handles);
          const params = decoder.decodeStructInline(blink.mojom.BadgeService_ClearBadge_ParamsSpec.$.structSpec);
