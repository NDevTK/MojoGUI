  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_DragTargetDragEnter_ParamsSpec.$.structSpec;
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for DragTargetDragEnter failed with TypeError');
           }
        }
        // Try Method 1: DragTargetDragOver
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_DragTargetDragOver_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for DragTargetDragOver failed with TypeError');
           }
        }
        // Try Method 2: DragTargetDragLeave
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_DragTargetDragLeave_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for DragTargetDragLeave failed with TypeError');
           }
        }
        // Try Method 3: DragTargetDrop
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_DragTargetDrop_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for DragTargetDrop failed with TypeError');
           }
        }
        // Try Method 4: DragSourceEndedAt
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_DragSourceEndedAt_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for DragSourceEndedAt failed with TypeError');
           }
        }
        // Try Method 5: DragSourceSystemDragEnded
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_DragSourceSystemDragEnded_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for DragSourceSystemDragEnded failed with TypeError');
           }
        }
        // Try Method 6: OnStartStylusWriting
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_OnStartStylusWriting_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for OnStartStylusWriting failed with TypeError');
           }
        }
        // Try Method 7: SetBackgroundOpaque
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_SetBackgroundOpaque_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> SetBackgroundOpaque (7)');
                 this.mapOrdinal(header.ordinal, 7);
                 dispatchId = 7;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetBackgroundOpaque failed with TypeError');
           }
        }
        // Try Method 8: SetTextDirection
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_SetTextDirection_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> SetTextDirection (8)');
                 this.mapOrdinal(header.ordinal, 8);
                 dispatchId = 8;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetTextDirection failed with TypeError');
           }
        }
        // Try Method 9: SetActive
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_SetActive_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> SetActive (9)');
                 this.mapOrdinal(header.ordinal, 9);
                 dispatchId = 9;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetActive failed with TypeError');
           }
        }
        // Try Method 10: SetInheritedEffectiveTouchActionForSubFrame
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_SetInheritedEffectiveTouchActionForSubFrame_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> SetInheritedEffectiveTouchActionForSubFrame (10)');
                 this.mapOrdinal(header.ordinal, 10);
                 dispatchId = 10;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetInheritedEffectiveTouchActionForSubFrame failed with TypeError');
           }
        }
        // Try Method 11: UpdateRenderThrottlingStatusForSubFrame
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_UpdateRenderThrottlingStatusForSubFrame_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> UpdateRenderThrottlingStatusForSubFrame (11)');
                 this.mapOrdinal(header.ordinal, 11);
                 dispatchId = 11;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for UpdateRenderThrottlingStatusForSubFrame failed with TypeError');
           }
        }
        // Try Method 12: SetIsInertForSubFrame
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_SetIsInertForSubFrame_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> SetIsInertForSubFrame (12)');
                 this.mapOrdinal(header.ordinal, 12);
                 dispatchId = 12;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetIsInertForSubFrame failed with TypeError');
           }
        }
        // Try Method 13: EnableDeviceEmulation
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_EnableDeviceEmulation_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> EnableDeviceEmulation (13)');
                 this.mapOrdinal(header.ordinal, 13);
                 dispatchId = 13;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for EnableDeviceEmulation failed with TypeError');
           }
        }
        // Try Method 14: DisableDeviceEmulation
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_DisableDeviceEmulation_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> DisableDeviceEmulation (14)');
                 this.mapOrdinal(header.ordinal, 14);
                 dispatchId = 14;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for DisableDeviceEmulation failed with TypeError');
           }
        }
        // Try Method 15: BindWidgetCompositor
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_BindWidgetCompositor_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> BindWidgetCompositor (15)');
                 this.mapOrdinal(header.ordinal, 15);
                 dispatchId = 15;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for BindWidgetCompositor failed with TypeError');
           }
        }
        // Try Method 16: SetViewportIntersection
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_SetViewportIntersection_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> SetViewportIntersection (16)');
                 this.mapOrdinal(header.ordinal, 16);
                 dispatchId = 16;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetViewportIntersection failed with TypeError');
           }
        }
        // Try Method 17: NotifyClearedDisplayedGraphics
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidget_NotifyClearedDisplayedGraphics_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (sizeMatch && (!!(header.flags & 1) === methodExpectsResp)) {
               if (size > 8 || message.payload.byteLength === 8) {
                 decoder.decodeStructInline(structSpec);
                 console.log('[GeneratedReceiver] Discovery SUCCESS: ' + header.ordinal + ' -> NotifyClearedDisplayedGraphics (17)');
                 this.mapOrdinal(header.ordinal, 17);
                 dispatchId = 17;
               }
             }
           } catch (e) {
             if (e instanceof TypeError) console.warn('[Discovery] trial for NotifyClearedDisplayedGraphics failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
      ...
      switch (dispatchId) {
        case 0: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              this.endpoint.send(header.ordinal, header.requestId, mojo.internal.kMessageFlagIsResponse, mojo.internal.bindings.blink.mojom.FrameWidget_DragTargetDragEnter_ResponseParamsSpec, response);
        ...
        case 1: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              this.endpoint.send(header.ordinal, header.requestId, mojo.internal.kMessageFlagIsResponse, mojo.internal.bindings.blink.mojom.FrameWidget_DragTargetDragOver_ResponseParamsSpec, response);
        ...
        case 3: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              this.endpoint.send(header.ordinal, header.requestId, mojo.internal.kMessageFlagIsResponse, mojo.internal.bindings.blink.mojom.FrameWidget_DragTargetDrop_ResponseParamsSpec, response);
        ...
        case 4: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              this.endpoint.send(header.ordinal, header.requestId, mojo.internal.kMessageFlagIsResponse, mojo.internal.bindings.blink.mojom.FrameWidget_DragSourceEndedAt_ResponseParamsSpec, response);
        ...
        case 6: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              this.endpoint.send(header.ordinal, header.requestId, mojo.internal.kMessageFlagIsResponse, mojo.internal.bindings.blink.mojom.FrameWidget_OnStartStylusWriting_ResponseParamsSpec, response);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidgetHost_AnimateDoubleTapZoomInMainFrame_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for AnimateDoubleTapZoomInMainFrame failed with TypeError');
           }
        }
        // Try Method 1: ZoomToFindInPageRectInMainFrame
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidgetHost_ZoomToFindInPageRectInMainFrame_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ZoomToFindInPageRectInMainFrame failed with TypeError');
           }
        }
        // Try Method 2: SetHasTouchEventConsumers
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidgetHost_SetHasTouchEventConsumers_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetHasTouchEventConsumers failed with TypeError');
           }
        }
        // Try Method 3: IntrinsicSizingInfoChanged
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidgetHost_IntrinsicSizingInfoChanged_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for IntrinsicSizingInfoChanged failed with TypeError');
           }
        }
        // Try Method 4: AutoscrollStart
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidgetHost_AutoscrollStart_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for AutoscrollStart failed with TypeError');
           }
        }
        // Try Method 5: AutoscrollFling
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidgetHost_AutoscrollFling_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for AutoscrollFling failed with TypeError');
           }
        }
        // Try Method 6: AutoscrollEnd
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.FrameWidgetHost_AutoscrollEnd_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for AutoscrollEnd failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
             console.warn('[GeneratedReceiver] Failed to discover ordinal ' + header.ordinal);
  ...
  bind(handle) {
    ...
    this.endpoint.start({ onMessageReceived: (...args) => {
      ...
      if (dispatchId === undefined) {
        ...
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.PopupWidgetHost_RequestClosePopup_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for RequestClosePopup failed with TypeError');
           }
        }
        // Try Method 1: ShowPopup
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.PopupWidgetHost_ShowPopup_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for ShowPopup failed with TypeError');
           }
        }
        // Try Method 2: SetPopupBounds
        if (dispatchId === undefined) {
           try {
             const structSpec = mojo.internal.bindings.blink.mojom.PopupWidgetHost_SetPopupBounds_ParamsSpec.$.structSpec;
             const size = decoder.decodeUint32(0);
             const version = decoder.decodeUint32(4);
             let sizeMatch = false;
             for (const v of structSpec.versions) {
             ...
             if (e instanceof TypeError) console.warn('[Discovery] trial for SetPopupBounds failed with TypeError');
           }
        }
        if (dispatchId === undefined) {
             console.warn('[GeneratedReceiver] Failed to discover ordinal ' + header.ordinal);
      ...
      switch (dispatchId) {
        ...
        case 1: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              this.endpoint.send(header.ordinal, header.requestId, mojo.internal.kMessageFlagIsResponse, mojo.internal.bindings.blink.mojom.PopupWidgetHost_ShowPopup_ResponseParamsSpec, response);
        ...
        case 2: {
          ...
          if (expectsResponse) {
            Promise.resolve(result).then(response => {
              this.endpoint.send(header.ordinal, header.requestId, mojo.internal.kMessageFlagIsResponse, mojo.internal.bindings.blink.mojom.PopupWidgetHost_SetPopupBounds_ResponseParamsSpec, response);
