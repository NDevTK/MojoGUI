...
export class CDPClient {
  ...
  async ensureMojoGUIPage() {
    ...
    if (!target) {
      ...
      if (anyPage) {
        console.error(
          "[CDP] MojoGUI not found, navigating to:",
          this.targetUrl,
        );
      ...
      console.error(
        "[CDP] Page exists but is crashed, reloading:",
        this.targetUrl,
      );
        ...
        .then(({ default: WebSocket }) => {
          ...
          this.wsConnection.on("open", async () => {
            try {
              ...
              console.error(
                `[CDP] Attached to target with session: ${this.sessionId}`,
              );
  ...
  _handleMessage(message) {
        ...
        const logEntry = {
          ...
          text: args
            .map((arg) =>
              arg.value !== undefined
                ? arg.value
                : arg.description || JSON.stringify(arg),
            )
            .join(" "),
          stackTrace,
        };
    ...
    if (message.method === "Target.targetCrashed") {
      ...
      if (targetId === this.targetId || !this.targetId) {
        console.error(
          `[CDP] Target crashed: ${targetId} (status: ${status}, errorCode: ${errorCode})`,
        );
    ...
    if (
      ...
      if (targetInfo.type === "page" && this._isMojoGUITarget(targetInfo)) {
        ...
        if (!this.targetId || this.crashed) {
          console.error(
            `[CDP] MojoGUI target discovered: ${targetInfo.targetId} (${targetInfo.url})`,
          );
          if (this.crashed) {
