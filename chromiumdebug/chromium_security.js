  ...
  try {
    ...
    Logger.info(
      "  Handler registered. " +
        ...
        " will run in new renderer processes.",
    );
    Logger.empty();
  } catch (e) {
...
class SymbolUtils {
  static getModuleByName(name) {
    try {
      for (var mod of host.currentProcess.Modules) {
        if (mod.Name.toLowerCase().indexOf(name.toLowerCase()) !== -1) {
          return mod;
        }
      }
    } catch (e) {}
  ...
  static getControl() {
    return host.namespace.Debugger.Utility.Control;
  }
    ...
    try {
      ...
      for (var line of output) {
        ...
        if (type) {
          ...
          var match = lineStr.match(
            /([0-9a-fA-F`]+)\s+([0-9a-fA-F]+)\s+.*chrome!(.+)/,
          );
          if (match) {
            results.push({
              type: type,
      ...
      Logger.debug("getVerboseSymbols failed: " + e.message);
    }
  ...
  static findSymbols(pattern) {
    ...
    try {
      ...
      Logger.debug("findSymbols failed: " + e.message);
    }
  ...
  static tryExecute(cmd, fallback = []) {
    try {
      return this.getControl().ExecuteCommand(cmd);
    } catch (e) {
      Logger.debug("tryExecute '" + cmd + "' failed: " + e.message);
      return fallback;
    }
  }

  static execute(cmd, debug = false) {
    try {
      var output = this.getControl().ExecuteCommand(cmd);
      var results = [];
      for (var line of output) {
        var lineStr = line.toString();
        if (debug) Logger.info("  [Cmd Output] " + lineStr);
        results.push(lineStr);
      }
      return results;
    } catch (e) {
      Logger.debug("execute '" + cmd + "' failed: " + e.message);
      return [];
    }
  }
  ...
  static evaluate(expression) {
    try {
      ...
      Logger.debug("evaluate failed: " + e.message);
    }
    return null;
  }
  ...
  static getSymbolName(subsysAddr, debug, includeInline) {
    ...
    try {
      ...
      for (var line of output) {
        ...
        var inlineMatch = lineStr.match(
          /Inlined function:\s+(.+?)\s+\([0-9a-fA-F]/,
        );
        if (inlineMatch) {
          inlinedSymbol = inlineMatch[1].trim();
          if (debug)
            Logger.info("  [Debug] Matched inlined symbol: " + inlinedSymbol);
          continue;
        }
        ...
        if (match && !outerSymbol) {
          outerSymbol = match[1];
          if (debug)
            Logger.info("  [Debug] Matched outer symbol: " + outerSymbol);
        }
      }
  ...
  static getReturnType(symbolOrAddr) {
    ...
    try {
      ...
      for (var line of output) {
        ...
        if (m) {
          var sig = m[1]
            .replace(/\s*(__cdecl|__stdcall|__fastcall|__thiscall)/g, "")
            .trim();
      ...
      Logger.debug("getReturnType dx fallback failed: " + e.message);
    }

    // 4. Inlined function fallback: query symbol via host JavaScript API
    try {
      var cleanSym = symbolName.includes("!")
        ? symbolName.split("!")[1]
        : symbolName;
      ...
      if (
        chromeModule &&
        chromeModule.Contents &&
        chromeModule.Contents.Symbols
      ) {
        try {
          var sym = chromeModule.Contents.Symbols.getValueAt(cleanSym);
          if (sym && sym.Type && sym.Type.functionReturnType) {
            var typeName =
              sym.Type.functionReturnType.Name ||
              sym.Type.functionReturnType.name;
            if (typeName) {
              if (typeName) {
                Logger.info(
                  "    [Type Detection] Host Symbol Type: " + typeName,
                );
          ...
          Logger.debug("getReturnType symbol access failed: " + symErr.message);
        }
      }
    } catch (e) {
      Logger.debug(
        "getReturnType inlined function fallback failed: " + e.message,
      );
    }
...
class MemoryUtils {
    ...
    try {
      ...
      if (addr) {
        ...
        for (var dline of dqOutput) {
          var dMatch = dline
            .toString()
            .match(/[0-9a-fA-F`]+\s+([0-9a-fA-F`]+)/);
          if (dMatch) return dMatch[1].replace(/`/g, "");
        }
      }
    } catch (e) {
      Logger.debug(
        "readGlobalPointer failed for " + symbolName + ": " + e.message,
      );
    }
    return null;
  }
  ...
  static parseBigInt(input) {
    if (typeof input === "bigint") {
      ...
      if (ptrStr.startsWith("0x") || ptrStr.startsWith("0X"))
        return BigInt(ptrStr);
  ...
  static getCppgcCageBase() {
    ...
    var val = this.readGlobalPointer(
      "chrome!cppgc::internal::CageBaseGlobal::g_base_",
    );
  ...
  static decompressCppgcPtr(compressedPtr, contextAddr) {
    ...
    if (contextAddr) {
      try {
        var context = BigInt(
          contextAddr.toString().startsWith("0x")
            ? contextAddr
            : "0x" + contextAddr,
        );
        base = context & kCageBaseMask;
      } catch (e) {
        Logger.debug("Context address parse failed: " + e.message);
      }
    }
  ...
  static writeStringImpl(implAddr, newString) {
    var ctl = SymbolUtils.getControl();
    var hexAddr = implAddr.toString().startsWith("0x")
      ? implAddr
      : "0x" + implAddr;
    ...
    try {
      ...
      Logger.debug("writeStringImpl header read failed: " + e.message);
    }
    ...
    try {
      ...
      for (var line of out) {
        ...
        else if (lineStr.includes("false") || lineStr.includes("0"))
          is8Bit = false;
      }
    } catch (e) {
    ...
    if (currentLen === 0) {
      Logger.warn(
        "  [Warning] Length detected as 0. This might be empty string OR read failure.",
      );
      ...
      if (newString.length > 0) {
        Logger.error(
          "Cannot overwrite: Existing string length appears to be 0 or unreadable.",
        );
      }
    ...
    if (!dataAddr) {
      try {
        var cmd =
          "dx &((WTF::StringImpl*)" + hexAddr + ")->" + charType + "()[0]";
        var out = ctl.ExecuteCommand(cmd);
        for (var line of out) {
          ...
          if (m) {
            ...
            if (offset >= 0n && offset < 100n) {
              // Header size is small
              ProcessCache.setOffset("WTF::StringImpl->" + charType, {
        ...
        Logger.debug("writeStringImpl dx characters failed: " + e.message);
      }
    }
    ...
    if (is8Bit) {
      for (var i = 0; i < newString.length; i++)
        bytes.push(newString.charCodeAt(i));
...
class BreakpointManager {
  static set(title, targets, description) {
    ...
    for (var t of targets) {
      ...
      var cmd =
        typeof t === "string" ? "bp " + t : t.cmd ? t.cmd : "bp " + t.sym;

      Logger.info(cmd + desc);
      try {
        ctl.ExecuteCommand(cmd);
      } catch (e) {
        Logger.debug("Breakpoint set failed for " + sym + ": " + e.message);
      }
    }
...
function isValidPointer(ptr) {
  ...
  if (str === "0" || str === "00000000" || str === "0000000000000000")
    return false;
  if (str === "0x0" || str === "0x00000000" || str === "0x0000000000000000")
    return false;
  return true;
}
  ...
  try {
    ...
    for (var line of out) {
      ...
      if (s.includes("Protect:")) {
        if (
          s.includes("PAGE_READWRITE") ||
          s.includes("PAGE_WRITECOPY") ||
          s.includes("PAGE_EXECUTE_READWRITE") ||
          s.includes("PAGE_EXECUTE_WRITECOPY")
        ) {
          return true;
        }
        return false;
...
function extractPointeeType(typeStr) {
  ...
  var match = typeStr.match(
    /(?:scoped_refptr|unique_ptr|Member|WeakMember|Persistent|CrossThreadPersistent|DataRef|RefPtr|base::RefCountedData)\s*<\s*([^<>,]+)/,
  );
  if (match) {
  ...
  match = typeStr.match(
    /^([a-zA-Z_][a-zA-Z0-9_:]*(?:\s+const)?)\s*\*?\s*&?\s*\*?\s*$/,
  );
  if (match) {
...
function getCompressedMember(baseAddr, typeCast, memberName) {
  ...
  if (className) {
    ...
    if (cached) {
      ...
      try {
        ...
        return host.memory.readMemoryValues(
          host.parseInt64(memberAddr.toString(16), 16),
          1,
          4,
        )[0];
      } catch (e) {
  ...
  try {
    var ctl = SymbolUtils.getControl();
    var cmd =
      "dx &(" + typeCast + "0x" + baseBig.toString(16) + ")->" + memberName;
    var out = ctl.ExecuteCommand(cmd);
    for (var line of out) {
      var l = line.toString();
      ...
      if (m) {
        ...
        try {
          var ptrVal = host.memory.readMemoryValues(
            host.parseInt64(addrStr, 16),
            1,
            4,
          )[0];
          return ptrVal;
        } catch (e) {
          Logger.error("ReadMem failed: " + e.message);
        }
      }
    }
  } catch (e) {
    Logger.debug("getCompressedMember failed: " + e.message);
  }
  return null;
}
...
function readUrlStringFromDx(addr, typeCast) {
  try {
    ...
    Logger.debug("readUrlStringFromDx failed: " + e.message);
  }
  return null;
}
      ...
      if (offsetFromData === -8 && i + 4 <= bytes.length) {
        ...
        if (i + 4 < bytes.length) {
          ...
          if ((flags & kIsAtomic) === kIsAtomic) {
            if (logFailures)
              Logger.info("  [DEBUG] Rejected Length at -8: Atomic String.");
            continue;
          }
        }
        ...
        if (i - 4 >= 0) {
          ...
          if (refCount === 0) {
            if (logFailures)
              Logger.info(
                "  [DEBUG] Rejected Length at -8: RefCount is 0 (Dead Object).",
              );
            continue;
          }
        }
...
function _patchStringInMemory(ctl, searchStr, replaceStr, label, isUnicode) {
  ...
  try {
    ...
    var searchCmd =
      "s " + cmdType + " 0 L?" + USER_MODE_ADDR_LIMIT + ' "' + searchStr + '"';
          ...
          if (headerInfo && headerInfo.valid) {
            ...
            if (actualOldLen > searchStr.length) {
              ...
              if (suffixLen < 5000) {
                ...
                var suffixAddr = addr.add(
                  searchStr.length * (isUnicode ? 2 : 1),
                );
                var suffixBytesCount = suffixLen * (isUnicode ? 2 : 1);
                try {
                  var suffixInts = host.memory.readMemoryValues(
                    suffixAddr,
                    Math.ceil(suffixBytesCount / 4),
                    4,
                  );
            ...
            finalNewLen =
              replaceStr.length + suffixBytes.length / (isUnicode ? 2 : 1);
            ...
            if (_isWritable(headerInfo.lengthAddr)) {
              if (headerInfo.encoding === "smi") {
                ...
                MemoryUtils.writeMemory(lenAddrStr, [
                  (finalNewLen << 1) & 0xff,
                ]);
            ...
            if (suffixBytes.length > 0) {
              var newSuffixAddr = addr.add(
                replaceStr.length * (isUnicode ? 2 : 1),
              );
              if (_isWritable(newSuffixAddr)) {
                MemoryUtils.writeMemory(
                  newSuffixAddr.toString(16),
                  suffixBytes,
                );
              }
            }
    ...
    if (skipped > 0 && !hasLoggedFailure) {
      Logger.info(
        "  " +
          ...
          " (Skipped " +
          skipped +
          " due to missing headers/strict mode)",
      );
...
class BlinkUnwrap {
  ...
  static getLocalFrame(webFrameAddr) {
    ...
    var frameCompressed = getCompressedMember(
      webFrameHex,
  ...
  static getDomWindow(localFrameAddr) {
    ...
    var windowCompressed = getCompressedMember(
      frameHex,
      "(blink::LocalFrame*)",
      "dom_window_",
    );
  ...
  static getDocument(domWindowAddr) {
    ...
    var docCompressed = getCompressedMember(
      windowHex,
  ...
  static getSecurityOriginUrl(documentAddr) {
    try {
      ...
      var cmd =
        "dx -r3 ((blink::Document*)" +
        docHex +
        ")->GetExecutionContext()->GetSecurityOrigin()->ToUrlOrigin().GetDebugString()";
      var output = ctl.ExecuteCommand(cmd);
      for (var line of output) {
        var lineStr = line.toString();
      ...
      return readUrlStringFromDx(
        documentAddr.replace(/^0x/, ""),
        "(blink::Document*)",
      );
    } catch (e) {}
    return null;
  ...
  static getCppMember(objectAddr, typeCast, memberPath) {
    ...
    try {
      ...
      for (var line of output) {
        ...
        if (
          typeStr.indexOf("BasicMember") !== -1 ||
          typeStr.indexOf("Member<") !== -1 ||
          typeStr.indexOf("scoped_refptr") !== -1 ||
          typeStr.indexOf("WeakMember") !== -1
        ) {
          // For pointer types, get the raw compressed value and decompress it
          try {
            var rawCmd =
              "dx &((" + typeCast + objHex + ")->" + memberPath + ".raw_)";
            ...
            if (rawAddr) {
              var compressedVal = host.memory.readMemoryValues(
                host.parseInt64(rawAddr, 16),
                1,
                4,
              )[0];
              if (compressedVal === 0) {
              ...
              var decompressed = MemoryUtils.decompressCppgcPtr(
                compressedVal,
                objHex,
              );
              if (decompressed && decompressed !== "0") {
          ...
          return {
            value: "(pointer type - use frame_attrs)",
            type: typeStr,
            raw: lineStr,
          };
        }
        ...
        if (match) {
          ...
          if (
            (typeStr.indexOf("String") !== -1 ||
              typeStr.indexOf("KURL") !== -1) &&
            (rawValue === "{...}" || rawValue.startsWith("0x"))
          ) {
            var strVal = this._readStringMember(
              objHex,
              typeCast,
              memberPath,
              typeStr,
            ...
            if (strVal !== null)
              return { value: '"' + strVal + '"', type: typeStr, raw: lineStr };
          }
  ...
  static setCppMember(objectAddr, typeCast, memberPath, value) {
    ...
    try {
      ...
      var isStringLike =
        memberType.indexOf("String") !== -1 ||
        memberType.indexOf("KURL") !== -1;
      ...
      if (isStringLike) {
        ...
        if (memberType.indexOf("AtomicString") !== -1) {
          Logger.warn(
            "[CAUTION] Mutating AtomicString. These are shared/unique across the process.",
          );
          Logger.warn(
            "          Changing this value may affect other objects!",
          );
        }

        // String type - find StringImpl and write characters
        var implCmd =
          "dx &((" + typeCast + objHex + ")->" + stringPath + ".impl_.ptr_)";
        ...
        if (implPtrAddr) {
          ...
          var implPtr = host.memory.readMemoryValues(
            host.parseInt64(implPtrAddr, 16),
            1,
            8,
          )[0];
        ...
        Logger.error(
          "Could not find StringImpl for string-like member: " + memberType,
        );
        return false;
        ...
        memberType.indexOf("unsigned int") !== -1 ||
        memberType.indexOf("uint32_t") !== -1 ||
        memberType.indexOf("int") !== -1 ||
        memberType.indexOf("uint16_t") !== -1 ||
        ...
        memberType.indexOf("uint64_t") !== -1 ||
        memberType.indexOf("size_t") !== -1 ||
        memberType.indexOf("uintptr_t") !== -1 ||
        memberType.indexOf("*") !== -1
      ) {
        ...
        Logger.info(
          "[C++] Unknown type '" + memberType + "', trying 32-bit write...",
        );
    ...
    try {
      ...
      for (var line of output) {
        ...
        if (colonMatch) {
          ...
          var value = typeMatch
            ? rest.replace(/\s*\[Type:[^\]]+\]\s*$/, "").trim()
            : rest;
          if (value.length === 0) value = "{...}";

          // DRY: Use _readStringMember for opaque string/URL members
          if (
            (type.indexOf("String") !== -1 || type.indexOf("KURL") !== -1) &&
        ...
        var noColonMatch = afterOffset.match(
          /^([a-zA-Z_][a-zA-Z0-9_]*)\s+\[Type:\s*([^\]]+)\]/,
        );
        if (noColonMatch) {
      ...
      Logger.debug("getCppMembers parse error: " + e.message);
    }
    return members;
  }
  ...
  static inspect(addr, options) {
    ...
    var isStringy =
      result.type &&
      (result.type.indexOf("String") !== -1 ||
        result.type.indexOf("KURL") !== -1);
    ...
    if (addrBig > 0x10000n && !result.stringValue) {
      ...
      if (isValidUserModePointer(val64)) {
        ...
        if (
          sym &&
          (sym.indexOf("vftable") !== -1 || sym.indexOf("??_7") !== -1)
        ) {
  ...
  static detectType(objectAddr, debug, noScan) {
    ...
    try {
      ...
      var vtablePtr = host.memory.readMemoryValues(
        host.parseInt64(objHex, 16),
        1,
        8,
      )[0];
      if (debug)
        Logger.info(
          "  [Debug] VTable Ptr at " + objHex + ": 0x" + vtablePtr.toString(16),
        );
      ...
      if (vtablePtr.compareTo(0) !== 0) {
        ...
        if (symName) {
          ...
          var matchVftable = symName.match(
            /!([a-zA-Z0-9_:]+)::(?:`vftable'|vftable)/,
          );
          if (matchVftable) {
            var className = matchVftable[1];
            if (debug)
              Logger.info("  [Debug] Detected Type (Std): " + className);
          ...
          if (!resultType) {
            ...
            if (matchMangled) {
              ...
              if (
                rawName.endsWith("blink") &&
                rawName.length > "blink".length
              ) {
                rawName = rawName.substring(0, rawName.length - "blink".length);
              }
              var fullType =
                rawName.indexOf("::") === -1 ? "blink::" + rawName : rawName;
              if (debug)
                Logger.info("  [Debug] Detected Type (Mangled): " + fullType);
              resultType = "(" + fullType + "*)";
            }
          }
          ...
          if (!resultType) {
            ...
            if (matchFallback) {
              ...
              if (debug)
                Logger.info("  [Debug] Detected Type (Fallback): " + className);
              resultType = "(" + className + "*)";
            }
          }
      ...
      if (debug)
        Logger.info("  [Debug] detectType direct check failed: " + e.message);
    }
      ...
      for (var line of output) {
        ...
        if (match) {
          ...
          if (className) {
            ...
            // Sanity check: Offset should be positive and reasonable
            if (calcedOffset >= 0n && calcedOffset < 0x10000n) {
              ProcessCache.setOffset(className + "->" + memberPath, {
                offset: calcedOffset.toString(),
                path: memberPath,
              });
            }
  ...
  static getFirstChild(nodeAddr) {
    ...
    var childCompressed = getCompressedMember(
      nodeHex,
      "(blink::ContainerNode*)",
      "first_child_",
    );
  ...
  static getNextSibling(nodeAddr) {
    var nodeHex = normalizeAddress(nodeAddr);
    if (!nodeHex) return null;
    var siblingCompressed = getCompressedMember(
      nodeHex,
      "(blink::Node*)",
      "next_",
    );
  ...
  static _extractAttributeValue(base) {
    ...
    try {
      ...
      for (var line of vOut4) {
        var lineStr = line.toString();
        if (
          lineStr.indexOf("value_") !== -1 ||
          lineStr.indexOf("Value") !== -1
        ) {
          var m = lineStr.match(/"([^"]+)"/);
          if (m) return m[1];
        }
      ...
      if (flags !== null) {
        ...
        if (type === 1) {
          var cmdE =
            "dx -r2 ((blink::Element*)" +
            nodeHex +
            ")->tag_name_.impl_->local_name_.string_";
          var res = BlinkUnwrap._parseStringFromDxOutput(
            ctl.ExecuteCommand(cmdE),
          );
          if (res) return res;
  ...
  static findMemberDeep(className, thisPtr, memberName) {
    ...
    while (queue.length > 0 && checks < maxChecks) {
      ...
      if (lookupClass.indexOf("!") === -1)
        lookupClass = "chrome!" + lookupClass;
      ...
      try {
        ...
        for (var line of output) {
          ...
          if (
            lineStr.indexOf("blink::") !== -1 &&
            (lineStr.indexOf("context_") !== -1 ||
              lineStr.indexOf("impl_") !== -1)
          ) {
            // Extract Member Name and Type
            // [+0x0f8] security_context_ [Type: blink::SecurityContext]
            var matchComp = lineStr.match(
              /\]\s*([a-zA-Z0-9_]+)\s*\[Type:\s*([a-zA-Z0-9_:]+)/,
            );
            if (matchComp) {
    ...
    Logger.info(
      "    [Deep Search] Failed after checking " + checks + " nodes.",
    );
    return null;
  }
  ...
  static _traverseAttributes(elementAddr, callback, debug) {
    ...
    var dataCompressed = getCompressedMember(
      elemHex,
      "(blink::Element*)",
      "element_data_",
    );
    ...
    if (debug) {
      ...
      Logger.info(
        "  [Debug] element_data_ compressed: " +
          (dataCompressed !== null
            ? "0x" + dataCompressed.toString(16)
            : "null"),
      );
    }
    ...
    if (!dataCompressed || dataCompressed === 0) {
      ...
      if (debug) {
        try {
          var dxCmd =
            "dx -r1 ((blink::Element*)" + elemHex + ")->element_data_";
    ...
    try {
      var cmd =
        "dx -r0 ((blink::ElementData*)0x" + dataAddr + ")->bit_field_.bits_";
      var output = ctl.ExecuteCommand(cmd);
      for (var line of output) {
        var lineStr = line.toString();
        if (
    ...
    if (isUnique === null) {
      if (debug)
        Logger.info("  [Debug] bitField detection failed, probing types...");

      // Try UniqueElementData first (more common for elements with attributes)
      try {
        var probeCmd =
          "dx -r0 ((blink::UniqueElementData*)0x" +
          dataAddr +
          ")->attribute_vector_[0]";
        ...
        for (var line of probeOut) {
          var lineStr = line.toString();
          if (
            lineStr.indexOf("[Type:") !== -1 &&
            lineStr.indexOf("Unable to") === -1
          ) {
            isUnique = true;
            if (debug)
              Logger.info("  [Debug] Detected UniqueElementData via probe");
            break;
          }
        }
      } catch (e) {}
      ...
      if (isUnique === null) {
        ...
        try {
          var probeCmd2 =
            "dx -r0 ((blink::ShareableElementData*)0x" +
            dataAddr +
            ")->attribute_array_[0]";
          ...
          for (var line of probeOut2) {
            ...
            if (
              ...
              if (debug)
                Logger.info(
                  "  [Debug] Detected ShareableElementData via probe",
                );
              break;
            }
          }
    ...
    if (isUnique === null) {
      if (debug)
        Logger.info(
          "  [Debug] Could not determine ElementData type - skipping",
        );
      return;
    }
    ...
    if (isUnique) {
      ...
      for (var probe = 0; probe < 100; probe++) {
        try {
          ...
          for (var line of probeOut) {
            var lineStr = line.toString();
            if (
              lineStr.indexOf("out of bounds") !== -1 ||
              lineStr.indexOf("Unable to") !== -1 ||
      ...
      if (debug)
        Logger.info("  [Debug] UniqueElementData count via probe: " + count);
    } else {
      ...
      if (debug)
        Logger.info(
          "  [Debug] ShareableElementData count from bitfield: " + count,
        );
    }
    ...
    for (var i = 0; i < count; i++) {
      try {
        var base = isUnique
          ? "((blink::UniqueElementData*)0x" +
            dataAddr +
            ")->attribute_vector_[" +
            i +
            "]"
          : "((blink::ShareableElementData*)0x" +
            dataAddr +
            ")->attribute_array_[" +
            i +
            "]";
        ...
        Logger.debug(
          "Attribute traversal error at index " + i + ": " + e.message,
        );
      }
    }
  }
  ...
  static inspectNode(nodeAddr) {
    ...
    try {
      ...
      Logger.error("Error dumping object: " + e.message);
    }
  }
}
...
function frame_setattr(objectAddr, memberName, value, typeHint) {
  if (isEmpty(objectAddr) || isEmpty(memberName)) {
    Logger.showUsage(
      "Set Object Member",
      "!frame_setattr <addr> <member> <value>",
      [
    ...
    Logger.info(
      "Works with: LocalFrame, Document, LocalDOMWindow, Element, Node, etc.",
    );
    Logger.empty();
    return "";
  }
  ...
  try {
    ...
    if (implAddr && implAddr !== "0") {
      ...
      Logger.warn(
        "[CAUTION] Mutating DOM attribute. These are AtomicStrings and often interned.",
      );
      Logger.warn(
        "          Changing this value may affect other elements with the same attribute value!",
      );
  ...
  var cppResult = BlinkUnwrap.getCppMemberWithFallback(
    objHex,
    member,
    typeHintStr,
  );
  if (cppResult) {
    ...
    var success = BlinkUnwrap.setCppMemberWithFallback(
      objHex,
      member,
      newValue,
      typeHintStr,
    ...
    if (success) {
      ...
      Logger.info(
        "Manual: dx &(" + cppResult.typeCast + objHex + ")->" + member,
      );
...
function frame_document(idx) {
  if (idx === undefined || idx === null || idx === "") {
    Logger.showUsage("Frame Document", "!frame_doc <frame_index>", [
      "!frame_doc 0",
      "!frame_doc 1",
    ]);
  ...
  if (!frame) {
    Logger.error(
      "Frame not found at index " + frameIdx + ". Use !frames to list frames.",
    );
    return "";
  }
...
function frame_window(idx) {
  if (idx === undefined || idx === null || idx === "") {
    Logger.showUsage("Frame Window", "!frame_win <frame_index>", [
      "!frame_win 0",
      "!frame_win 1",
    ]);
    Logger.info("Use !frames to list all frames with indices.");
...
function frame_origin(idx) {
  if (idx === undefined || idx === null || idx === "") {
    Logger.showUsage("Frame Origin", "!frame_origin <frame_index>", [
      "!frame_origin 0",
      "!frame_origin 1",
    ]);
    Logger.info("Use !frames to list all frames with indices.");
...
function frame_elements(idx, tagName) {
  if (idx === undefined || idx === null || idx === "") {
    Logger.showUsage("Frame Elements", "!frame_elem <frame_index> [tag_name]", [
      "!frame_elem 0",
      '!frame_elem 0 "div"',
      '!frame_elem 1 "iframe"',
    ]);
  ...
  try {
    if (filterTag) {
      ...
      Logger.info(
        "  dx ((blink::Document*)0x" + document + ")->documentElement()",
      );
...
function frame_getattr(objectAddr, memberName, typeHint) {
  ...
  if (cppResult) {
    ...
    if (valMatch) {
      ...
      var inspection = BlinkUnwrap.inspect(valHex, {
        typeHint: extractPointeeType(cppResult.type),
      });
...
function frame_attrs(objectAddr, debug, typeHint) {
  ...
  var addrBig = 0n;
  try {
    addrBig = MemoryUtils.parseBigInt(objHex);
  } catch (e) {}
  ...
  if (!isValidUserModePointer(addrBig) && addrBig !== 0n) {
    Logger.error(
      "Address " +
        objHex +
        " appears to be a compressed pointer that could not be decompressed.",
    );
    Logger.info("This may happen if the CppGC cage base is not yet available.");
    Logger.info(
      "Try: !v8_cage to check cage bases, or provide a full 64-bit address.",
    );
    Logger.empty();
    return "";
  }
  ...
  var attrs = [];
  try {
    BlinkUnwrap._traverseAttributes(
      objHex,
      (name, base) => {
        var valStr = BlinkUnwrap._extractAttributeValue(base);
        attrs.push({ name: name, value: valStr || "" });
  ...
  var result = BlinkUnwrap.getCppMembersWithFallback(
    objHex,
    enableDebug,
    typeHintStr,
  );
  ...
  var inspection = BlinkUnwrap.inspect(objHex, {
    debug: enableDebug,
    typeHint: activeType,
  });
                ...
                if (decompressed && decompressed !== "0") {
                  ...
                  Logger.info(
                    "  " +
                      m.name +
                      " -> 0x" +
                      ptrValBig.toString(16) +
                      " (compressed?)  !frame_getattr(" +
                      objHex +
                      ', "' +
                      m.name +
                      '", "' +
  ...
  if (result.members.length === 0) {
    if (inspection.isPointer && inspection.pointerTarget && addrBig > 0) {
      ...
      Logger.info(
        "  [" +
          inspection.pointerType +
          " -> " +
          inspection.pointerTarget +
          "]",
      );
      Logger.empty();
    ...
    if (addrBig === 0n) {
      ...
      if (activeType) {
        ...
        Logger.info(
          "  Provide type: !frame_attrs(" +
            objHex +
            ', false, "(blink::TypeName*)")',
        );
      }
    }
  } else {
...
class ProcessUtils {
  ...
  static parseInfo(cmdLine) {
    ...
    if (cmdLine.toLowerCase().indexOf("chrome.exe") !== -1) {
      if (
        cmdLine.indexOf("--monitor-self") !== -1 ||
        cmdLine.indexOf("crashpad") !== -1
      )
        return { type: "crashpad-handler", extra: "" };
      if (
        cmdLine.indexOf("--enable-features") !== -1 &&
        cmdLine.indexOf("--field-trial-handle") !== -1
      )
        return { type: "browser", extra: "" };
      if (
        cmdLine.length < BROWSER_CMDLINE_MIN_LENGTH ||
        cmdLine.indexOf("--user-data-dir") !== -1
      )
        return { type: "browser", extra: "" };
    }
  ...
  static forEachProcess(filterType, callback) {
    ...
    for (var proc of processes) {
      ...
      try {
        ...
        Logger.error("Error in process " + proc.pid + ": " + e.message);
      }
    }
    // Restore original process context
    try {
...
class Exec {
  ...
  static _detectTypeFromOffset(targetSymbol, offset) {
    ...
    var cleanSym = targetSymbol.includes("!")
      ? targetSymbol.split("!")[1]
      : targetSymbol;
    ...
    if (memberName) {
      ...
      if (typeInfo) {
        Logger.info(
          "    [Type Detection] dt offset 0x" +
            offsetHex.toUpperCase() +
            " -> " +
            memberName +
            " : " +
            typeInfo.typeName,
  ...
  static _parseChain(cmdString) {
    ...
    for (var i = 0; i < cmdString.length; i++) {
      ...
      if (!inQuote) {
        ...
        if (depth === 0) {
          var isArrow =
            c === "-" && i + 1 < cmdString.length && cmdString[i + 1] === ">";
          var isDot = c === ".";
  ...
  static _execChain(calls) {
    ...
    for (var i = 0; i < calls.length; i++) {
      ...
      if (
        currentThis &&
        typeof currentThis === "string" &&
        currentThis.startsWith("0x")
      ) {
  ...
  static _resolveAutoThis(className, frameIndex = 0) {
    ...
    if (this._forcedContext) {
      Logger.info(
        "    [Auto-This] Attempting resolution from pinned context: " +
          this._forcedContext,
      );
      ...
      if (detected) {
        ...
        if (detectedName === className) {
          Logger.info(
            "    [Auto-This] Pinned context matches " +
              className +
              ": " +
              this._forcedContext,
          );
    ...
    var resolvers = {
      ...
      "blink::LocalFrame": function (lf) {
        return lf;
      },
      "blink::Frame": function (lf) {
        return lf;
      }, // LocalFrame IS-A Frame
      "blink::LocalDOMWindow": function (lf) {
        return BlinkUnwrap.getDomWindow(lf);
      },
      "blink::DOMWindow": function (lf) {
        return BlinkUnwrap.getDomWindow(lf);
      }, // LocalDOMWindow IS-A DOMWindow
      "blink::ExecutionContext": function (lf) {
        return BlinkUnwrap.getDomWindow(lf);
      },
    ...
    if (!localFrame) {
      Logger.debug(
        "    [Auto-This] Could not get LocalFrame from WebLocalFrameImpl",
      );
      return null;
    }
    ...
    if (result) {
      ...
      var logResult = result.toString().startsWith("0x")
        ? result
        : "0x" + result;
      Logger.info(
        "    [Auto-This] Using " +
  ...
  static _execSingle(entry, thisOverride, previousHint) {
    ...
    if (
      ...
      if (/^0x[0-9a-fA-F`]+$/.test(trimEntry)) {
        ...
        Logger.info(
          "    [Segment] Detected raw address. Pining context: " + addr,
        );
          ...
          if (detectedType) {
            ...
            Logger.error(
              "Cannot resolve method '" +
                methodPart +
                "' without type information/symbol.",
            );
            Logger.info(
              '    Provide fully qualified name, e.g., !exec "0x...->chrome!Class::Method(...)"',
            );
            return null;
          }
        } else {
    ...
    if (
      ...
      if (!detectedType && previousHint) {
        ...
        Logger.info(
          "    [Chain] Using type from previous call: " + detectedType,
        );
      }
      ...
      if (detectedType) {
        ...
        Logger.info(
          "    [Chain Auto-Detect] " + thisOverride + " -> " + targetSymbol,
        );
      } else {
        Logger.error(
          "    [Chain] Type detection failed for '" + namePart + "'.",
        );
        Logger.info(
          "    Please use explicit type qualification: e.g. 'blink::ClassName::" +
            namePart +
            "'",
        );
        return null;
    ...
    if (detectedReturnType) {
      Logger.info(
        "    [Type Detection] Programmatically detected: " + detectedReturnType,
      );
    ...
    if (!this.currentReturnTypeHint) {
      ...
      if (inferredType) {
        Logger.info(
          "    [Type Inference] Inferred return type (Fallback): " +
            inferredType,
        );
    ...
    if (symInfo && symInfo.type === "inline_candidates") {
      Logger.info(
        "    [Chain] Found " +
          symInfo.candidates.length +
          " inlined candidates for '" +
          targetSymbol +
          "'",
      );
      ...
      for (var cand of symInfo.candidates) {
        ...
        if (!this._checkRelocatable(cand.address, cand.size)) {
          Logger.warn(
            "    [Chain] Skipping non-relocatable candidate @ " + cand.address,
          );
          continue;
        }
      ...
      if (bestCandidate) {
        Logger.info(
          "    [Chain] Selected best inlined candidate @ " +
            bestCandidate.address +
            " (Offset: 0x" +
            bestOffset.toString(16) +
            ")",
        );

        // Inferred return type using DRY helper
        var inlinedSymbol =
          bestCandidate.foundClass + "::" + bestCandidate.foundMethod;
        var inlinedReturnType = SymbolUtils.getReturnType(
          "chrome!" + inlinedSymbol,
        );
        if (inlinedReturnType) {
          this.currentReturnTypeHint =
            this._normalizeTypeHint(inlinedReturnType);
          if (this.currentReturnTypeHint) {
            Logger.info(
              "    [Type Detection] Inlined return type: " +
                this.currentReturnTypeHint,
            );
          }
        }
        ...
        if (thisPtr && bestCandidate.foundClass !== targetClass) {
          var adjustedThis = this._adjustThisPointer(
            thisPtr,
            "chrome!" + inlinedSymbol,
          );
        ...
        return this._executeInlinedCode(
          bestCandidate.address,
          bestCandidate.size,
          args,
          bestRegs.inputReg,
        ...
        Logger.error(
          "    [Chain] Could not find a suitable inlined candidate.",
        );
        return null;
      }
    }
    ...
    if (symInfo && symInfo.type === "inline") {
      ...
      if (!this._checkRelocatable(symInfo.address, symInfo.size)) {
        Logger.error(
          "    [Inline] Code contains control flow or RIP-relative addressing. Cannot relocate safely.",
        );
        return null;
      }
      ...
      if (regs.offset > 0) {
        var detectedType = this._detectTypeFromOffset(
          targetSymbol,
          regs.offset,
        );
        if (detectedType) {
    ...
    if (!thisPtr && args.length === 0 && targetSymbol.includes("::")) {
      ...
      Logger.warn(
        "Running method without 'this' pointer (and Auto-This failed).",
      );
      Logger.warn(
        "If this is an instance method, it will likely crash or return 0.",
      );
      Logger.warn(
        "Ensure you are in a valid frame with 'g_frame_map' or provide explicit 'this'.",
      );
    }
  ...
  static _execViaDx(symbol, argsStr, thisPtr) {
    Logger.info(
      "    [Direct] Symbol not found, attempting direct member access...",
    );
    ...
    Logger.info(
      "    [Deep Search] Searching hierarchy for " + memberName + "...",
    );
    ...
    if (!hit) {
      ...
      if (className.indexOf("!") === -1) {
        hit = BlinkUnwrap.findMemberDeep(
          "chrome!" + className,
          thisPtr,
          memberName,
        );
    ...
    if (memberOffset === null) {
      ...
      Logger.error(
        "Cannot find backing member '" + memberName + "' for " + methodName,
      );
      Logger.info("    Class: " + className);
      Logger.info(
        "    Make sure to provide the EXACT member name (e.g. 'url_' not 'Url')",
      );
  ...
  static _adjustThisPointer(thisPtr, targetSymbol) {
    try {
      ...
      var originalClass = originalType
        .replace(/[()*]/g, "")
        .trim()
        .replace(/^chrome!/, "");
        ...
        for (var line of output) {
          ...
          if (m) {
            ...
            if (adjustedPtr !== thisPtr && adjustedPtr !== "0x0") {
              ...
              Logger.info(
                "    [Multi-Inheritance] Adjusted 'this': " +
                  thisPtr +
                  " -> " +
                  adjustedPtr +
                  " (Offset: " +
  ...
  static _execViaInline(className, methodName, thisPtr) {
    ...
    for (var i = 0; i < patterns.length; i++) {
      ...
      try {
          ...
          for (var sym of symbols) {
            ...
            if (pattern === "chrome!*::" + methodName) {
              ...
              if (allowedNs.indexOf(symNs) === -1) {
                Logger.debug(
                  "    [PDB] Skipping unrelated namespace: " + symNs,
                );
                continue;
              }
            }
            ...
            if (isExactMatch && sym.type === "func") {
              ...
              Logger.info(
                "    [PDB] Found exact non-inlined: " + exactFuncAddr,
              );
            }
        ...
        Logger.debug("    [PDB] Pattern lookup failed: " + e.message);
      }
    }
    ...
    if (inlineCandidates.length > 0) {
      ...
      for (var cand of orderedCandidates) {
        ...
        if (!this._checkRelocatable(cand.addr, cand.size)) {
          Logger.warn(
            "    [PDB] Skipping (contains control flow): " + cand.addr,
          );
          continue;
        }
        ...
        var inlinedReturnType =
          cand.returnType ||
          SymbolUtils.getReturnType("chrome!" + inlinedSymbol);
        if (inlinedReturnType) {
        ...
        Logger.info(
          "    [PDB] Executing real inlined code from " +
            cand.addr +
            " (" +
            cand.size +
            " bytes)",
        ...
        if (cand.foundClass !== className) {
          adjustedThis = this._adjustThisPointer(
            thisPtr,
            "chrome!" + inlinedSymbol,
          );
        }
        ...
        var inlineArgs = [
          { type: "int", realValue: MemoryUtils.parseBigInt(adjustedThis) },
        ];
        return this._executeInlinedCode(
          cand.addr,
          cand.size,
      ...
      Logger.warn(
        "    [PDB] All " +
          ...
          " inline candidates contain control flow",
      );
      if (exactFuncAddr) {
        Logger.info(
          "    [PDB] Falling back to non-inlined version @ " + exactFuncAddr,
        );
        return this._execViaInline_Part2(exactFuncAddr, thisPtr);
      }

      Logger.warn(
        "    [PDB] No relocatable inline found and no non-inlined function available (falling back to member lookup)",
      );
    ...
    Logger.error(
      "    [PDB] No PDB entry found for " + className + "::" + methodName,
    );
    return null;
  }
  ...
  static _execViaVtable(thisPtr, className, methodName) {
    ...
    try {
      ...
      var vtablePtrRaw = host.memory.readMemoryValues(
        host.parseInt64(thisBig.toString(16), 16),
        1,
        8,
      )[0];
      ...
      if (vtablePtr < 0x10000n) {
        Logger.warn(
          "    [Vtable] Invalid vtable pointer: 0x" + vtablePtr.toString(16),
        );
        return null;
      }
      ...
      if (vtablePtr > 0x7fffffffffffn) {
        Logger.warn(
          "    [Vtable] Vtable pointer out of user-mode range: 0x" +
            vtablePtr.toString(16),
        );
        return null;
      }
      ...
      try {
        ...
        Logger.debug("    [Vtable] Slot resolution failed: " + e.message);
      }
      ...
      if (!isValidVtable) {
        Logger.warn(
          "    [Vtable] Address 0x" +
            vtablePtr.toString(16) +
            " is not a valid vtable (class may not be polymorphic)",
        );
        return null;
      }
      ...
      if (!funcAddr) {
        try {
          var dqsOut = ctl.ExecuteCommand(
            "dqs 0x" + vtablePtr.toString(16) + " L20",
          );
          var slotIdx = 0;
      ...
      if (!funcAddr) {
        Logger.warn(
          "    [Vtable] Method " + methodName + " not found in vtable",
        );
        return null;
      }
      ...
      Logger.info(
        "    [Vtable] Calling virtual method at 0x" + funcAddr.toString(16),
      );

      var args = [{ type: "int", realValue: thisBig }];
      return this._runX64(
        "0x" + funcAddr.toString(16),
        args,
        this.currentReturnTypeHint,
      );
    } catch (e) {
  ...
  static _getMemberOffset(className, memberName) {
    ...
    try {
      var qualifiedClass =
        className.indexOf("!") === -1 ? "chrome!" + className : className;
      var output = ctl.ExecuteCommand(
        "dt " + qualifiedClass + " " + memberName,
      );
      for (var line of output) {
        var lineStr = line.toString();
      ...
      Logger.debug("    [Offset] dt command failed: " + e.message);
    }
    return null;
  }
  ...
  static _getMemberAtOffset(className, offset) {
    ...
    try {
      ...
      for (var line of output) {
        ...
        if (m) {
          ...
          if (Math.abs(memberOffset - offset) <= 4) {
            Logger.debug(
              "    [Offset Check] Found '" + m[2] + "' at 0x" + m[1],
            );
      ...
      Logger.debug("    [MemberAtOffset] dt lookup failed: " + e.message);
    }
    return null;
  }
    ...
    try {
      ...
      for (var line of output) {
        ...
        if (m) {
          ...
          var isComplexObject =
            !isPointer && !isReference && !isPrimitive && !isBitfield;
  ...
  static _inferReturnType(methodName, className) {
    ...
    if (
      /^(Get)?(Name|Title|Href|Value|Id|String|Text|Message)$/i.test(
        methodName,
      ) ||
    ...
    if (
      ...
      if (
        !/^(int|bool|float|double|void|size|length|count)$/.test(methodName)
      ) {
  ...
  static _readMemberDirect(thisPtr, className, memberName) {
    ...
    if (cached) {
      Logger.info(
        "    [Cache] Reading member at offset 0x" + cached.offset.toString(16),
      );
      return this._readCachedMember(cached, thisPtr);
    }
    ...
    if (typeInfo) {
      Logger.info(
        "    [TypeInfo] Using PDB type: " +
          typeInfo.typeName +
          " (complex=" +
          typeInfo.isComplexObject +
          ")",
      );
      ...
      if (typeInfo.isComplexObject) {
        ...
        Logger.info(
          "    [Type Hint] Set hint for next call: " + typeInfo.typeName,
        );
    ...
    try {
      ...
      var expr =
        ...
        className +
        "*)0x" +
        MemoryUtils.parseBigInt(thisPtr).toString(16) +
        ")->" +
        memberName;
        ...
        try {
          ...
          for (var line of offsetOut) {
            ...
            if (m) {
              ...
              Logger.info(
                "    [Cache] Stored offset 0x" + m[1] + " for " + memberName,
              );
              break;
            }
          }
      ...
      for (var line of output) {
        ...
        if (isFirstLine) {
          ...
          if (typeMatch) {
            ...
            if (
              typeName !== "bool" &&
              lineStr.indexOf(" : ") === -1 &&
              !lineStr.match(/:\s*(true|false|\d+|0x)/)
            ) {
              Logger.info(
                "    [Fallback] Detected complex object: " + typeName,
              );

              // Set return type hint for next chain execution
              this.currentReturnTypeHint = typeName;
              Logger.info(
                "    [Type Hint] Set hint for next call: " + typeName,
              );
              ...
              for (var offLine of offsetOut) {
                ...
                if (m) {
                  ...
                  var addrStr = "0x" + addr.toString(16);
                  Logger.info(
                    "    [Fallback] Result (Object Address): " + addrStr,
                  );
                  return addrStr;
                }
              }
        ...
        if (lineStr.match(/\[Type:.*(?:String|AtomicString)/i)) {
          var strContent = BlinkUnwrap._readStringMember(
            thisPtr,
            "(" + className + "*)",
            memberName,
            "String",
        ...
        if (intMatch) {
          ...
          if (!val.startsWith("0x"))
            val = "0x" + parseInt(val, 10).toString(16);
        ...
        var enumMatch = lineStr.match(
          /:\s*(\w+)\s*\((\d+|0x[0-9a-fA-F]+)\)\s*\[Type:/,
        );
        if (enumMatch) {
          var enumVal = enumMatch[2];
          if (!enumVal.startsWith("0x"))
            enumVal = "0x" + parseInt(enumVal, 10).toString(16);
          Logger.info(
            "    [Fallback] Result (enum): " +
              enumMatch[1] +
              " (" +
              enumVal +
              ")",
  ...
  static _getInlinedRegs(inlineAddr) {
    ...
    try {
      ...
      for (var line of uOutput) {
        ...
        var leaMatch = lineStr.match(
          /lea\s+(\w+),.*?\[(\w+)(?:\+([0-9a-fA-F]+)h?)?\]/i,
        );
        if (leaMatch) {
        ...
        var movssMatch = lineStr.match(
          /movs[sd]\s+(\w+),.*?\[(\w+)(?:\+([0-9a-fA-F]+)h?)?\]/i,
        );
        if (movssMatch) {
        ...
        var movMatch = lineStr.match(
          /mov\s+(\w+),.*?\[(\w+)(?:\+([0-9a-fA-F]+)h?)?\]/i,
        );
        if (movMatch) {
        ...
        if (addMatch) {
          ...
          Logger.info(
            "    [PDB] Pattern: ADD " + inputReg + ", 0x" + offset.toString(16),
          );
          break;
        }

        // MOVSXD (compressed pointer logic): movsxd rcx,dword ptr [rbx+28h]
        var movsxdMatch = lineStr.match(
          /movsxd\s+(\w+),.*?\[(\w+)(?:\+([0-9a-fA-F]+)h?)?\]/i,
        );
        if (movsxdMatch) {
  ...
  static _checkRelocatable(addr, size) {
    ...
    try {
      ...
      for (var line of output) {
        ...
        if (
          /\s(call|jmp|je|jne|jz|jnz|jg|jge|jl|jle|ja|jae|jb|jbe|jo|jno|js|jns)\s/.test(
            lineStr,
          )
        ) {
          Logger.warn(
            "    [RelocationCheck] Found control flow: " + lineStr.trim(),
          );
          return false;
        }
        ...
        if (/\[rip[+\-]/i.test(lineStr)) {
          Logger.warn(
            "    [RelocationCheck] Found RIP-relative addressing: " +
              lineStr.trim(),
          );
          return false;
        }
  ...
  static _execViaInline_Part2(exactFuncAddr, thisPtr) {
    ...
    if (exactFuncAddr) {
      Logger.info(
        "    [PDB] Using exact class non-inlined function @ " + exactFuncAddr,
      );
  ...
  static _prepareStringArg(str, scratchBase, currentOffset) {
    ...
    var empty8 = MemoryUtils.readGlobalPointer(
      "chrome!blink::StringImpl::empty_",
    );
    var empty16 = MemoryUtils.readGlobalPointer(
      "chrome!blink::StringImpl::empty16_bit_",
    );
    ...
    if (!empty16) {
      empty16 = MemoryUtils.readGlobalPointer(
        "chrome!WTF::StringImpl::empty16_bit_",
      );
    }
    ...
    if (is8Bit) {
      if (!empty8) {
        Logger.error(
          "    [StringView] Cannot resolve StringImpl::empty_ - string argument preparation will fail!",
        );
        Logger.error(
          "    [StringView] This is required for passing string arguments to Blink functions.",
        );
        Logger.info(
          "    [StringView] Try running: dx chrome!blink::StringImpl::empty_",
        );
        throw new Error(
          "StringImpl::empty_ not found - cannot construct 8-bit StringView",
        );
      }
      ...
      if (!empty16) {
        Logger.error(
          "    [StringView] Cannot resolve StringImpl::empty16_bit_ - string argument preparation will fail!",
        );
        throw new Error(
          "StringImpl::empty16_bit_ not found - cannot construct 16-bit StringView",
        );
      }
    ...
    if (is8Bit) {
      ...
      for (var j = 0; j < str.length; j++) {
        ...
        charBytes.push(code & 0xff);
        charBytes.push((code >> 8) & 0xff);
      }
    ...
    var implBytes = [];
    var tempImpl = implPtrVal;
    for (var m = 0; m < 8; m++) {
      implBytes.push(Number(tempImpl & 0xffn));
      tempImpl >>= 8n;
    }
    ...
    var ptrBytes = [];
    var tempPtr = strDataAddr;
    for (var k = 0; k < 8; k++) {
      ptrBytes.push(Number(tempPtr & 0xffn));
      tempPtr >>= 8n;
    }
    ...
    Logger.info(
      "    [StringView] Struct at: 0x" +
        viewAddr.toString(16) +
        " (impl=0x" +
        implPtrVal.toString(16) +
        ", len=" +
        str.length +
        ")",
    );
  ...
  static _prepareAllArgs(args, scratchBase, currentOffset) {
    for (var i = 0; i < args.length; i++) {
      if (args[i].type === "string") {
        if (!scratchBase)
          throw new Error("Scratch memory required for string argument");
        var result = this._prepareStringArg(
          args[i].value,
          scratchBase,
          currentOffset,
        );
        ...
        if (!scratchBase)
          throw new Error("Scratch memory required for array argument");
        var result = this._prepareArrayArg(
          args[i].value,
          scratchBase,
          currentOffset,
        );
  ...
  static _executeInlinedCode(
    ...
    if (validRegs.indexOf(outputReg) === -1) {
      Logger.warn(
        "    Invalid outputReg '" + outputReg + "', defaulting to rax",
      );
      outputReg = "rax";
    }
    ...
    try {
      ...
      if (needsScratch) {
        ...
        try {
          currentScratchOffset = this._prepareAllArgs(
            args,
            scratchBase,
            currentScratchOffset,
          );
        } catch (e) {
      ...
      if (srcAddr < 0x10000n || srcAddr > BigInt("0x7FFFFFFFFFFF")) {
        Logger.error(
          "    [Sanity Check] Source address 0x" +
            srcAddr.toString(16) +
            " is outside valid range!",
        );
        Logger.info(
          "    This may indicate stale cache. Try: ProcessCache.clearCurrent()",
        );
        return null;
      }
      ...
      var codeBytes = [];
      try {
        var values = host.memory.readMemoryValues(
          host.parseInt64("0x" + srcAddr.toString(16)),
          inlineSize,
          1,
        );
        for (var v of values) {
        ...
        Logger.error(
          "    [Code Copy] Failed to read from 0x" +
            srcAddr.toString(16) +
            ": " +
            readErr.message,
        );
      ...
      if (codeBytes.length > 0) {
        var allZeros = codeBytes
          .slice(0, Math.min(4, codeBytes.length))
          .every((b) => b === 0);
        var allInt3 = codeBytes
          .slice(0, Math.min(4, codeBytes.length))
          .every((b) => b === 0xcc);
        if (allZeros || allInt3) {
          Logger.warn(
            "    [Sanity Check] Copied code looks invalid (all zeros or INT3)",
          );
          Logger.info(
            "    First bytes: " +
              codeBytes
      ...
      if (!codeInvalid && codeBytes.length >= 4) {
        ...
        if (!looksLikeValidCode) {
          Logger.warn(
            "    [Pattern Check] First instruction doesn't match expected getter pattern",
          );
          Logger.info(
            "    First bytes: " +
              codeBytes
      ...
      try {
        ...
        for (var dLine of disasm) {
          ...
          if (
            retOffset === null &&
            (/\sret[nf]?\s*$/.test(lineStr) || /\sret\s/.test(lineStr))
          ) {
      ...
      if (DEBUG_MODE) {
        try {
          ...
          Logger.info(
            "    [Debug] Stopped at RIP: 0x" +
              ripVal +
              " (offset 0x" +
              offsetInBuf.toString(16) +
              " in buffer)",
          );
      ...
      if (resultVal >= bufBig && resultVal < bufBig + BigInt(bufSize)) {
        Logger.warn(
          "    [WARNING] Result points to execution buffer. Register corruption suspected.",
        );
        Logger.warn(
          "    This usually means 'this' was not passed correctly in " +
            inputReg,
        );
      }
  ...
  static _returnPointer(thisPtr, offset) {
    ...
    Logger.info(
      "    Pointer: 0x" + thisAddr.toString(16) + " + 0x" + offset.toString(16),
    );
  ...
  static _readMember(thisPtr, offset) {
    ...
    try {
      var value = host.memory.readMemoryValues(
        host.parseInt64(memberAddr.toString(16), 16),
        1,
        8,
      )[0];
  ...
  static _analyzeResult(resultVal, floatValBitcast) {
    ...
    if (floatValBitcast !== undefined && floatValBitcast !== 0n) {
      try {
        ...
        if (view.setBigUint64) {
          ...
          var isFloatNormal =
            (Math.abs(fVal) > 1e-38 && Math.abs(fVal) < 1e38) || fVal === 0;
    ...
    var isBoolHint =
      this.currentReturnTypeHint &&
      this.currentReturnTypeHint.toLowerCase() === "bool";
    ...
    var skipBoolCheck =
      this.currentReturnTypeHint && this.currentReturnTypeHint !== "bool";
    if (resultVal > 0x10000n) skipBoolCheck = true;

    if (
      !skipBoolCheck &&
      (lowByte === 0n || lowByte === 1n) &&
      upperBytes !== 0n &&
      !isFloat
    ) {
      var boolStr = lowByte === 1n ? "true" : "false";
      Logger.info(
        "    Result (bool heuristic): " +
          boolStr +
          " (0x" +
          lowByte.toString(16) +
          ")",
    ...
    if (!isFloat && resultVal > 0x10000n) {
      ...
      if (high32 !== 0n && !isValidUserModePointer(resultVal)) {
        if (this.currentThis && low32 > 0n) {
          ...
          try {
            ...
            var context = BigInt(
              this.currentThis.toString().startsWith("0x")
                ? this.currentThis
                : "0x" + this.currentThis,
            );
            ...
            if (isValidUserModePointer(directResult)) {
              Logger.info(
                "    [Pointer recovered] 0x" +
                  resultVal.toString(16) +
                  " -> 0x" +
                  directResult.toString(16),
              );
      ...
      var looksLikeNegativeInt =
        resultVal >= 0xffff0000n && resultVal <= 0xffffffffn;
      if (
        resultVal <= 0xffffffffffn &&
        this.currentThis &&
        !looksLikeNegativeInt
      ) {
        try {
          var decompressed = MemoryUtils.decompressCppgcPtr(
            resultVal,
    ...
    if (!isFloat && resultVal > 0x10000n) {
      var inspection = BlinkUnwrap.inspect(resultVal, {
        typeHint: this.currentReturnTypeHint,
      });
      ...
      if (
        inspection.type &&
        (inspection.type.indexOf("char*") !== -1 ||
          inspection.type.indexOf("char *") !== -1) &&
        inspection.stringValue === null
      ) {
        try {
  ...
  static _processArg(arg) {
    ...
    Logger.warn(
      "    [Args] Unrecognized argument format: '" +
        arg +
        "' - treating as literal",
    );
    return { type: "int", value: arg };
  ...
  static _runX64(targetAddr, args, returnType = null) {
    ...
    try {
      currentDataOffset = this._prepareAllArgs(
        args,
        baseAddr,
        currentDataOffset,
      );
    ...
    if (isStructReturn && args.length > 0) {
      ...
      Logger.debug(
        "    [Struct Return] Using Chrome/Clang convention: RCX=this, RDX=buffer",
      );
      Logger.debug(
        "    [Struct Return] Return buffer at: 0x" +
          structReturnAddr.toString(16),
      );
    ...
    try {
      ...
      var resultVals = host.memory.readMemoryValues(
        host.parseInt64(addrStr),
        2,
        8,
      );
      ...
      Logger.error(
        "Failed to read result from execution buffer: " + memReadErr.message,
      );
      Logger.error(
        "This usually means the shellcode execution crashed or caused an exception.",
      );
        ...
        for (var line of ripOut) {
          ...
          if (/^[0-9a-fA-F]+$/.test(s)) {
            ...
            if (ripBig >= baseAddr && ripBig < baseAddr + BigInt(allocSize)) {
              ...
              Logger.info(
                "  [Diagnostic] Crash in shellcode buffer at offset 0x" +
                  offset.toString(16),
              );
            } else {
      ...
      try {
        ...
        for (var line of exrOut) {
          var lineStr = line.toString();
          if (
            lineStr.indexOf("ExceptionCode:") !== -1 ||
            lineStr.indexOf("ExceptionAddress:") !== -1
          ) {
      ...
      try {
        ...
        for (var line of raxOut) {
          ...
          if (/^[0-9a-fA-F]+$/.test(s)) {
            result = BigInt("0x" + s);
            Logger.info(
              "  Fallback: Read RAX directly = 0x" + result.toString(16),
            );
            break;
          }
        }
    ...
    if (isStructReturn) {
      ...
      Logger.info(
        "  [Struct Return] Reading result from buffer: " + structAddrHex,
      );
      ...
      if (returnType.includes("StringView")) {
        ...
        try {
          ...
          var savedVals = host.memory.readMemoryValues(
            host.parseInt64(baseAddrHex, 16),
            1,
            8,
          );
          ...
          if (raxVal === structReturnAddr) {
            ...
            var structData = host.memory.readMemoryValues(
              host.parseInt64(structAddrHex, 16),
              3,
              8,
            );
            ...
            var lengthData = host.memory.readMemoryValues(
              host.parseInt64((structReturnAddr + 16n).toString(16), 16),
              1,
              4,
            );
            length = lengthData[0];
            ...
            dataPtr = raxVal;
            var rdxVals = host.memory.readMemoryValues(
              host.parseInt64((baseAddr + 16n).toString(16), 16),
              1,
              8,
            );
          ...
          if (dataPtr > 0x10000n && length > 0 && length < 10000) {
            var strResult = "";
            if (is8Bit) {
              var chars = host.memory.readMemoryValues(
                host.parseInt64(dataPtr.toString(16), 16),
                length,
                1,
              );
...
function help() {
  ...
  Logger.info(
    "  !frame_getattr(el,a)  - Get DOM attribute OR C++ member (auto-detect)",
  );
  Logger.info(
    "  !frame_setattr(el,a,v)- Set DOM attribute OR C++ member (auto-detect)",
  );
  Logger.info(
    "  !frame_attrs(el)      - List all DOM attributes AND C++ members",
  );
  Logger.empty();
...
function decompress_gc(ptr) {
  if (isEmpty(ptr)) {
    Logger.showUsage(
      ...
      [
        "!decompress_gc 0x12345678",
        "Used for blink::Member<T>, cppgc::internal::BasicMember<T>",
      ],
    );
    return "";
  }
  ...
  if (result === null) {
    ...
    Logger.warn("Could not decompress - cage base not found.");
    Logger.info(
      "Try !decompress_gc <ptr> <context_address> to derive base from an object.",
    );
    Logger.empty();
  } else {
    Logger.empty();
...
function get_site_locks(browserSysId, childIds) {
  ...
  var funcAddr = SymbolUtils.findSymbolAddress(
    "chrome!content::ChildProcessSecurityPolicyImpl::GetInstance",
  );
  ...
  ProcessUtils.forEachProcess("browser", function (proc) {
    try {
      ...
      for (var lmLine of lmOut) {
        ...
        if (
          lmStr.indexOf("chrome") !== -1 &&
          lmStr.indexOf("start") === -1 &&
          lmStr.indexOf("Browse") === -1
        ) {
          hasChrome = true;
          break;
        }
          ...
          try {
            ...
            for (var poiLine of poiOut) {
              ...
              if (poiMatch) {
                ...
                if (
                  isValidPointer(ptrVal) &&
                  ptrVal.length > MIN_PTR_VALUE_LENGTH
                ) {
                  ...
                  var memoryOk = false;
                  try {
                    var dqsCheck = SymbolUtils.execute(
                      "dqs " + candidateAddr + " L1",
                    );
                    for (var dqsLine of dqsCheck) {
                      var dqsStr = dqsLine.toString();
                      if (
                        dqsStr.indexOf("????????") === -1 &&
                        dqsStr.indexOf(ptrVal.substring(0, 8)) !== -1
                      ) {
                        memoryOk = true;
                      }
                    }
  ...
  try {
    ...
    var enumCmd =
      "dx -r2 ((chrome!content::ChildProcessSecurityPolicyImpl*)" +
      instanceAddr +
...
function chrome_process_type() {
  ...
  if (info.type === "renderer") {
    try {
      ...
      Logger.debug("renderer_site failed: " + e.message);
    }
  }
  ...
  for (var pInfo of processInfoList) {
    ...
    if (pInfo.type === "renderer" && pInfo.clientId) {
      ...
      if (g_spoofMap.has(pInfo.clientId)) {
        displayExtra +=
          " [SPOOFED: " + g_spoofMap.get(pInfo.clientId).currentUrl + "]";
      }
    }
...
function renderer_site() {
  ...
  if (g_spoofMap.has(clientId)) {
    ...
    Logger.info(
      "  Spoofed Site:       " +
        g_spoofMap.get(clientId).currentUrl +
        " (Active)",
    );
  } else {
...
function mojo_interfaces() {
  ...
  if (info.type !== "renderer") {
    ...
    Logger.empty();
    Logger.info(
      "Use !procs to list processes, then |<id>s to switch to a renderer.",
    );
    Logger.empty();
    return "";
  }
  ...
  try {
    ...
    Logger.info(
      "Filtering for renderer client ID: " +
        clientId +
        " (PID: " +
        rendererPid +
        ")",
    ...
    try {
      ...
      var dxCmd =
        "dx -r5 (*((content::`anonymous namespace'::RoutingIDFrameMap*)0x" +
        mapPtr +
        "))";
      ...
      for (var line of dxOutput) {
        ...
        var rfhMatch =
          ...
          lineStr.match(
            /RenderFrameHostImpl\s*\*?\s*[=:]\s*(0x[0-9a-fA-F`]+)/i,
          );
        if (rfhMatch) {
    ...
    Logger.info(
      "Found " +
        rfhAddresses.length +
        " RenderFrameHostImpl(s) for this renderer",
    );
    Logger.empty();
  ...
  Logger.info(
    "Found " + interfaces.length + " mojo interface(s) exposed to renderer:",
  );
  Logger.info("-".repeat(70));
  Logger.empty();
  ...
  Logger.info(
    "  https://source.chromium.org/chromium/chromium/src/+/main:content/browser/browser_interface_binders.cc",
  );
  Logger.empty();
...
function patch_function(funcName, returnValue) {
  ...
  try {
    ...
    for (var line of out) {
      if (
        line.toString().toLowerCase().indexOf("x64") !== -1 ||
        line.toString().indexOf("AMD64") !== -1
      ) {
        isX64 = true;
        break;
      }
    ...
    if (!isX64) {
      Logger.error(
        "Patching is currently only supported on x64 architectures.",
      );
  ...
  if (isEmpty(returnValue)) {
    ...
    returnValue.toString().startsWith("0x") ||
    returnValue.toString().startsWith("0X")
  ) {
  ...
  try {
    ...
    for (var sym of symbols) {
      ...
      try {
        ...
        Logger.error(
          "  [FAILED] " + sym.name + " @ 0x" + sym.addr + ": " + e.message,
        );
      }
    }
    ...
    Logger.info(
      "  NOTE: Patches are direct code modifications (not breakpoints)",
    );
    Logger.info(
      "  TIP: V8 caches results - navigate to a new page to see effect in JS",
    );
    Logger.empty();
    ...
    if (count > 0 && funcName.indexOf("!") === -1) {
      ...
      if (callers.length > 0) {
        Logger.info(
          "  Found " +
            callers.length +
            " potential caller(s) that may inline this function.",
        );
        Logger.info(
          '  If patch doesn\'t work, try: !patch("<caller_name>","' +
            retVal +
            '")',
        );
...
function on_process_exit() {
  ...
  try {
    ...
    for (var id of idsToRemove) {
      ...
      Logger.info(
        "\n  [Cleanup] Spoof state cleaned up for PID " +
          currentPid +
          " (Url: " +
          spoof.currentUrl +
          ")",
...
function spoof_origin(arg1, arg2, arg3) {
  ...
  if (isEmpty(arg1)) {
    ...
    Logger.info(
      '    !spoof("https://target.com")              - Spoof to target origin',
    );
    Logger.info(
      '    !spoof("https://target.com", "host")      - Replace host only',
    );
    Logger.info(
      '    !spoof("https://target.com", "scheme")    - Replace scheme only',
    );
    Logger.info(
      '    !spoof("https://target.com", "full")      - Replace full origin URL',
    );
    Logger.empty();
    Logger.info("  Generic String Replacement:");
    Logger.info(
      '    !spoof("oldstring", "newstring", "string")  - Replace ASCII string',
    );
    Logger.info(
      '    !spoof("oldstring", "newstring", "unicode") - Replace Unicode string',
    );
    Logger.empty();
    Logger.info("  Examples:");
    Logger.info(
      '    !spoof("https://evil.com")                - Spoof origin to evil.com',
    );
    Logger.info(
      '    !spoof("example.com", "evil.com", "string") - Replace all \'example.com\'',
    );
    Logger.info(
      '    !spoof("secret", "public", "string")      - Replace \'secret\' with \'public\'',
    );
    Logger.empty();
    Logger.info(
      "  Note: Replacement must be <= search length to prevent buffer overflow.",
    );
    Logger.empty();
    return "";
  }
  ...
  if (mode === "string" || mode === "unicode") {
    ...
    Logger.info(
      '  Search:  "' + searchStr + '" (len=' + searchStr.length + ")",
    );
    Logger.info(
      '  Replace: "' + replaceStr + '" (len=' + replaceStr.length + ")",
    );
    Logger.empty();

    var isUnicode = mode === "unicode";
    var patched = _patchStringInMemory(
      ctl,
      searchStr,
      replaceStr,
      "String",
  ...
  if (!targetMatch) {
    Logger.warn(
      "Invalid URL format. Use scheme://host (e.g., https://example.com)",
    );
    return "";
  }
  ...
  try {
    var cmdLine = getCommandLine();
    var clientMatch = cmdLine
      ? cmdLine.match(/--renderer-client-id=(\d+)/)
      : null;
    if (clientMatch) clientId = clientMatch[1];
  ...
  if (!trueOrigin) {
    ...
    Logger.info(
      'For generic string replacement, use: !spoof("old", "new", "string")',
    );
    Logger.empty();
    return "";
  }
  ...
  var currentMatch = currentOrigin.match(
    /^([a-zA-Z][a-zA-Z0-9+.-]*):\/\/(.+)$/,
  );
  if (!currentMatch) {
  ...
  switch (mode) {
    case "scheme":
      if (currentScheme !== targetScheme) {
        totalPatched += _patchStringInMemory(
          ctl,
          currentScheme,
          targetScheme,
          "Scheme",
  ...
  try {
    host.currentProcess.on("exit", function () {
      Logger.info(
        "  [Cleanup] Process " +
          pid +
          " exiting. Clearing related spoofing entries.",
      );
    ...
    Logger.debug(
      "Failed to register per-PID exit handler for " + pid + ": " + e.message,
    );
  }
}
...
function sandbox_status_all() {
  ...
  Logger.info(
    "  ID    PID       Type              Integrity Level           Status",
  );
  ...
  for (var proc of processList) {
    try {
      ...
      if (type === "browser") {
        ...
        status =
          integrity.indexOf("Medium") !== -1 || integrity.indexOf("High") !== -1
            ? "OK"
            : "WARN";
      } else if (type === "renderer") {
        // Renderer MUST be Low, Untrusted, or AppContainer
        if (
          integrity.indexOf("Untrusted") !== -1 ||
      ...
      if (integrity.length > MAX_INTEGRITY_DISPLAY_LENGTH)
        integrity =
          integrity.substring(0, MAX_INTEGRITY_DISPLAY_LENGTH - 3) + "...";
...
function site_isolation_status() {
  ...
  var flags = {
    ...
    "disable-site-isolation":
      cmdLine.indexOf("--disable-site-isolation") !== -1,
  ...
  Logger.info(
    "    --site-per-process:          " +
      (flags["site-per-process"] ? "ENABLED" : "not set"),
  );
  Logger.info(
    "    --disable-site-isolation:    " +
      (flags["disable-site-isolation"] ? "WARNING: DISABLED" : "not set"),
  );
  Logger.info(
    "    --isolate-origins:           " +
      (flags["isolate-origins"] ? "ENABLED" : "not set"),
  );
  ...
  Logger.info("  " + "-".repeat(40));
  Logger.info(
    "    bp chrome!content::SiteIsolationPolicy::UseDedicatedProcessesForAllSites",
  );
...
function _parseFrameMapEntries(ctl, mapAddr) {
  ...
  var dxCmd =
    "dx -r5 *((content::`anonymous namespace'::FrameMap*)0x" + mapAddr + ")";
  var mapOutput = ctl.ExecuteCommand(dxCmd);

  for (var line of mapOutput) {
    var lineStr = line.toString();
    var firstMatch = lineStr.match(
      /first\s*:\s*(0x[0-9a-fA-F`]+)\s*\[Type:\s*blink::WebFrame/i,
    );
    if (firstMatch) {
      currentWebFrame = firstMatch[1].replace(/`/g, "");
    }
    var secondMatch = lineStr.match(
      /second\s*:\s*(0x[0-9a-fA-F`]+)\s*\[Type:\s*content::RenderFrameImpl/i,
    );
...
function _extractFrameUrl(ctl, f) {
  ...
  if (frameCompressed !== null) {
    localFrameAddr = MemoryUtils.decompressCppgcPtr(
      frameCompressed,
      f.webFrame,
    );
  }
  ...
  var loaderCompressed = getCompressedMember(
    normalizeAddress(localFrameAddr),
    "(blink::LocalFrame*)",
    "loader_",
  );
  if (loaderCompressed !== null) {
    var loaderAddr = MemoryUtils.decompressCppgcPtr(
      loaderCompressed,
      localFrameAddr,
    );
    if (isValidPointer(loaderAddr)) {
      var docLoaderCompressed = getCompressedMember(
        normalizeAddress(loaderAddr),
        "(blink::FrameLoader*)",
        "document_loader_",
      );
      if (docLoaderCompressed !== null) {
        var docLoaderAddr = MemoryUtils.decompressCppgcPtr(
          docLoaderCompressed,
          loaderAddr,
        );
        if (isValidPointer(docLoaderAddr)) {
          var url = readUrlStringFromDx(
            docLoaderAddr.toString(16),
  ...
  if (windowCompressed !== null) {
    var windowAddr = MemoryUtils.decompressCppgcPtr(
      windowCompressed,
      localFrameAddr,
    );
    if (isValidPointer(windowAddr)) {
      var docCompressed = getCompressedMember(
        normalizeAddress(windowAddr),
        "(blink::LocalDOMWindow*)",
        "document_",
      );
...
function renderer_frames() {
  ...
  try {
    ...
    if (!mapAddr) {
      Logger.info(
        "  LazyInstance not yet initialized (no frames created yet).",
      );
      Logger.empty();
      return "";
    }
    ...
    if (frames.length > 0) {
      ...
      Logger.info(
        "    dx *((content::`anonymous namespace'::FrameMap*)0x" +
          mapAddr +
          ")",
      );
      Logger.empty();
    ...
    Logger.info(
      "    dx ((content::RenderFrameImpl*)<addr>)         - Inspect RenderFrameImpl",
    );
    Logger.info(
      "    dx ((content::RenderFrameImpl*)<addr>)->frame_ - Get WebLocalFrame",
    );
    Logger.info(
      "    dx ((blink::WebLocalFrame*)<addr>)             - Inspect WebFrame",
    );
    Logger.empty();
  } catch (e) {
    ...
    Logger.info(
      "    dq <addr> L1                ; Read LazyInstance.private_instance_",
    );
...
function _runInProcessType(command, processType, exampleCmd) {
  if (isEmpty(command)) {
    ...
    Logger.empty();
    Logger.info(
      "  Works from ANY process - automatically finds and runs in " +
        processType +
        ".",
    );
    Logger.empty();
...
function run_in_renderer(command) {
  return _runInProcessType(
    command,
    "renderer",
    "bp chrome!v8::internal::Heap::CollectGarbage",
  );
}
...
function run_in_gpu(command) {
  return _runInProcessType(
    command,
    "gpu-process",
    "bp chrome!gpu::CommandBufferService::Flush",
  );
}
...
function on_renderer_attach(command) {
  ...
  if (isEmpty(command)) {
    Logger.info(
      "  This sets up commands to run when new renderer processes attach.",
    );
    Logger.empty();
    ...
    Logger.info(
      '    !on_renderer_attach "bp chrome!blink::Document::CreateRawElement"',
    );
  ...
  Logger.info(
    "  Total registered commands: " + g_rendererAttachCommands.length,
  );
  Logger.empty();
...
function run_script_in_renderer(scriptPath) {
  if (isEmpty(scriptPath)) {
    ...
    Logger.empty();
    Logger.info(
      "  Works from ANY process - automatically loads in all renderers.",
    );
    Logger.empty();
    return "";
  }
...
function script_in_renderer_attach(scriptPath) {
  ...
  if (isEmpty(scriptPath)) {
    ...
    Logger.empty();
    Logger.info(
      "  This will auto-load the script when new renderer processes spawn.",
    );
    Logger.empty();
    Logger.info("  Examples:");
    Logger.info('    !script_in_renderer_attach "renderer_hooks.js"');
    Logger.info(
      '    !script_in_renderer_attach "C:\\research\\exploit_test.js"',
    );
    Logger.empty();
    return "";
  }
  ...
  _registerRendererSxeHandler(
    '!run_script_in_renderer "' + escapedPath + '"',
    "Script",
  );
...
function bp_security_check() {
  return set_breakpoints(
    ...
    [
      {
        sym: "chrome!content::ChildProcessSecurityPolicyImpl::CanAccessDataForOrigin",
        desc: "Origin access check",
      },
      {
        sym: "chrome!content::ChildProcessSecurityPolicyImpl::CanCommitURL",
        desc: "URL commit check",
      },
      {
        sym: "chrome!content::ChildProcessSecurityPolicyImpl::GetProcessLock",
        desc: "Process lock query",
      },
      {
        sym: "chrome!content::SiteIsolationPolicy::UseDedicatedProcessesForAllSites",
        desc: "Site isolation check",
      },
      {
        sym: "chrome!content::SiteInstanceImpl::GetProcess",
        desc: "SiteInstance process",
      },
      {
        sym: "chrome!sandbox::TargetServicesBase::LowerToken",
        desc: "Sandbox token lowering",
      },
    ],
...
function trace_ipc() {
  return set_breakpoints(
    ...
    [
      {
        sym: "chrome!mojo::MessageDispatcher::Accept",
        desc: "Mojo message accept",
        cmd: 'bp chrome!mojo::MessageDispatcher::Accept ".echo [IPC] Mojo message accept; k 3; g"',
      },
      {
        sym: "chrome!content::BrowserInterfaceBrokerImpl::GetInterface",
        desc: "Interface broker",
        cmd: 'bp chrome!content::BrowserInterfaceBrokerImpl::GetInterface ".echo [IPC] Interface broker; k 3; g"',
      },
      {
        sym: "chrome!IPC::ChannelMojo::OnMessageReceived",
        desc: "Legacy IPC receive",
        cmd: 'bp chrome!IPC::ChannelMojo::OnMessageReceived ".echo [IPC] Legacy IPC receive; k 3; g"',
      },
    ],
    ...
    var categories = [
      {
        ...
        targets: [
          {
            sym: "ntdll!RtlFreeHeap",
            desc: "Heap free (watch for double-free)",
          },
          { sym: "chrome!base::PartitionFree", desc: "PartitionAlloc free" },
          {
            sym: "chrome!content::RenderProcessHostImpl::Cleanup",
            desc: "Renderer cleanup",
          },
        ],
      },
      {
        ...
        targets: [
          {
            sym: "chrome!blink::V8ScriptValueDeserializer::Deserialize",
            desc: "Deserialization",
          },
          {
            sym: "chrome!v8::internal::Object::ToObject",
            desc: "V8 type coercion",
          },
        ],
      },
      {
        ...
        targets: [
          {
            sym: "chrome!content::ChildProcessHost::OnMessageReceived",
            desc: "IPC receive (browser)",
          },
          {
            sym: "chrome!content::ChildThreadImpl::OnMessageReceived",
            desc: "IPC receive (renderer)",
          },
        ],
      },
      {
        ...
        targets: [
          {
            sym: "chrome!base::debug::BreakDebugger",
            desc: "Debug break (crash)",
          },
          {
            sym: "chrome!base::debug::CollectStackTrace",
            desc: "Stack trace (crash path)",
          },
        ],
      },
      {
...
function heap_info() {
  ...
  try {
    ...
    Logger.info(
      "    !heap -a <addr>                    - Analyze heap address",
    );
    Logger.info(
      "    dps <addr> L10                     - Dump pointers at address",
    );
...
/// =============================================================================
/// MOJO HANDLE RESEARCH & HIJACKING
/// =============================================================================

/**
 * List all blink::MojoHandle objects on the heap.
 * These are the handles that JavaScript can access.
 */

/**
 * Optimized memory search helper (Smart Search).
 * Merges adjacent committed regions into large chunks for speed.
 */
function get_search_ranges() {
  var ranges = [];
  try {
    // 1. Get ONLY Private Read/Write committed regions (Actual Heap)
    var regions = host.currentProcess.Memory.Regions.Where(
      (r) => r.State == 0x1000 && r.Protect == 0x04 && r.Type == 0x20000,
    ).OrderBy((r) => r.BaseAddress);

    // 2. Merge adjacent regions to minimize search calls
    var currentBase = null;
    var currentSize = host.parseInt64(0);

    for (var region of regions) {
      if (currentBase === null) {
        currentBase = region.BaseAddress;
        currentSize = region.Size;
      } else {
        var end = currentBase.add(currentSize);
        if (end.compareTo(region.BaseAddress) === 0) {
          // Adjacent! Extend the current range
          currentSize = currentSize.add(region.Size);
        } else {
          // Gap! Save and start new range
          ranges.push(
            currentBase.toString(16) + " L?" + currentSize.toString(16),
          );
          currentBase = region.BaseAddress;
          currentSize = region.Size;
        }
    ...
    if (currentBase !== null) {
      ranges.push(currentBase.toString(16) + " L?" + currentSize.toString(16));
    }
  } catch (e) {
    // Broad fallback that covers your high-address hit (0x1084...)
    ranges = ["0 L?0x7fffffff0000"];
  }
  return ranges;
}

/**
 * List all blink::MojoHandle objects (JavaScript-exposed handles).
 */
function list_mojo_js_handles() {
  Logger.section("Mojo JS Handles");

  var jsVtable = SymbolUtils.findSymbolAddress(
    "chrome!blink::MojoHandle::`vftable'",
  );
  if (!jsVtable) {
    Logger.error("Failed to resolve blink::MojoHandle vtable.");
    return "";
  }
  ...
  for (var line of results) {
    ...
    try {
      ...
      var handleVal = SymbolUtils.evaluate("poi(0x" + addr + "+0x10)");
      var handleHex = handleVal.toString(16).toLowerCase();

      Logger.info(
        "[JS Handle] Address: 0x" + addr + " | Handle Value: 0x" + handleHex,
      );
      count++;
    } catch (e) {}
  }
...
function hijack_interface(jsHandleAddr, interfaceName) {
  ...
  var cleanJsAddr = jsHandleAddr.toString().replace("0x", "").replace("'", "");
  var cleanName = interfaceName.toString().replace(/"/g, "");

  if (cleanName.indexOf(".") == -1 && !cleanName.startsWith("blink.mojom.")) {
    cleanName = "blink.mojom." + cleanName;
  }

  Logger.section("Mojo Interface Hijack: " + cleanName);

  // 1. Resolve Name String Address
  var chromeMod = SymbolUtils.getModuleByName("chrome.dll");
  var start = chromeMod.BaseAddress.toString(16).replace("0x", "");
  var searchString = '"' + cleanName + '"';

  Logger.info("Resolving '" + cleanName + "'...");
  var strResults = SymbolUtils.execute(
    "s -a 0x" +
      start +
      " L?0x" +
      chromeMod.Size.toString(16) +
      " " +
      searchString,
  );

  if (strResults.length === 0) {
    return Logger.error("Interface string not found in chrome.dll.");
  }

  var strAddr = SymbolUtils.extractAddress(strResults[0]);
  Logger.info("Found name at 0x" + strAddr);

  // 2. Find Owner (Endpoint Client) & Traversal
  // We use the JS handle's current peer address as a "hint" for where the heap is.
  var hintVal = SymbolUtils.evaluate("poi(0x" + cleanJsAddr + "+0x10)");
  var hintHex = hintVal.toString(16).padStart(16, "0");
  var hintBase = hintHex.substring(0, hintHex.length - 8) + "00000000";

  var ranges = [
    hintBase + " L?0x7fffffff", // Scan the region where we know Mojo objects are
    "0 L?0x7fffffffffff", // Fallback to full scan
  ];

  var clientAddr = null;
  var masterHandleVal = 0;
  var interfaceId = 0;

  for (var region of ranges) {
    if (clientAddr) break;
    Logger.info("Scanning region " + region + "...");
    var ptrs = SymbolUtils.execute("s -q " + region + " " + strAddr, true);

    for (var line of ptrs) {
      var hit = SymbolUtils.extractAddress(line);
      if (!hit) continue;

      try {
        // base = InterfaceEndpointClient (StringPtr is at +0x1B8)
        var clientBase = host.parseInt64(hit, 16).subtract(0x1b8);

        // In your session, InterfaceEndpointClient + 0x0C8 leads to the Endpoint
        // We actually want the Endpoint object directly which we found is usually the "base" returned by some searches.
        // Based on your dt: Endpoint + 0x48 points back to the client.
        // So we look for Endpoint where +0x48 == clientBase.

        var endpoints = SymbolUtils.execute(
          "s -q " + region + " " + clientBase.toString(16),
          true,
        );
        for (var eLine of endpoints) {
          var eHit = SymbolUtils.extractAddress(eLine);
          var endpointAddr = host.parseInt64(eHit, 16).subtract(0x48);

          // Validate Endpoint
          // +0x18 is the Interface ID (id_)
          var idRead = SymbolUtils.execute(
            "dd 0x" + endpointAddr.toString(16) + "+0x18 L1",
          );
          if (idRead.length > 0) {
            var id = parseInt(idRead[0].trim().split(/\s+/).pop(), 16);
            if (id === 0 || id > 0xffffff) continue; // Sanity check

            // +0x10 is the controller_ pointer
            var controllerPtr = SymbolUtils.evaluate(
              "poi(0x" + endpointAddr.toString(16) + "+0x10)",
            );

            // Controller + 0x38 is the connector_ (unique_ptr)
            var connector = SymbolUtils.evaluate(
              "poi(0x" + controllerPtr.toString(16) + "+0x38)",
            );

            // Connector + 0x10 is the message_pipe_ (Master Handle)
            var masterRead = SymbolUtils.execute(
              "dd 0x" + connector.toString(16) + "+0x10 L1",
            );
            if (masterRead.length > 0) {
              var master = parseInt(
                masterRead[0].trim().split(/\s+/).pop(),
                16,
              );

              if (master > 0 && master < 0x7fffffff) {
                clientAddr = clientBase;
                masterHandleVal = master;
                interfaceId = id;
                Logger.info("Found Associated Link Path:");
                Logger.info("  Endpoint:   0x" + endpointAddr.toString(16));
                Logger.info("  Controller: 0x" + controllerPtr.toString(16));
                Logger.info("  Connector:  0x" + connector.toString(16));
                break;
              }
            }
          }
  ...
  if (!clientAddr) {
    Logger.error(
      "FATAL: Could not traverse Mojo object graph for " + cleanName,
    );
    return "";
  }

  // 3. Perform the Hijack
  try {
    // Write the Master Handle to the JS object
    var writeCmd =
      "ed 0x" + cleanJsAddr + "+0x10 0x" + masterHandleVal.toString(16);
    SymbolUtils.execute(writeCmd);

    Logger.header("HIJACK SUCCESS");
    Logger.info("Master Handle injected: 0x" + masterHandleVal.toString(16));
    Logger.info("Interface ID (for MojoGUI): " + interfaceId);
    Logger.info("Required Payload in MojoGUI Console:");
    Logger.info("  const master = ... (your hijacked handle name)");
    Logger.info(
      "  const router = new mojo.internal.interfaceSupport.Router(master);",
    );
    Logger.info(
      "  const endpoint = new mojo.internal.interfaceSupport.Endpoint(router, " +
        interfaceId +
        ");",
    );
    Logger.info("  const remote = new " + cleanName + "Remote(endpoint);");
  } catch (e) {
